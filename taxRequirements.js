const uuid = require("uuid");
const paf = require("./PISPauth")
const {setRequestBody} = require("./requestBodies");
const {format} = require("date-fns");
const {setAuthType} = require("./PISPauth");
let unixDate = new paf.UnixDate()

let authType = "PAUapikey" // PAUapikey, OBclientCredentials
paf.setAuthType("PAUapikey")
process.env["NODE_TLS_REJECT_UNAUTHORIZED"] = 0;

async function generateSignaturePOSTpaymentConsentstaxRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,additionalInfo = []){
    let header = {"alg": "BIGNS128",
  "crit": [
    "http://openbanking.asb.by/asn1",
    "http://openbanking.asb.by/crptPrvdr",
    "http://openbanking.asb.by/signDtTm",
    "http://openbanking.asb.by/signedData",
    // "http://openbanking.asb.by/debtorIdentification"
  ],
  "http://openbanking.asb.by/asn1": true,
  "http://openbanking.asb.by/crptPrvdr": 1,
  "http://openbanking.asb.by/signDtTm": unixDate.getISOWithTimeZone(unixDate.getDateNsecondsAgo(-10800)),
  "http://openbanking.asb.by/signedData": {
    "pars": [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "content-type",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
  },
  // "http://openbanking.asb.by/debtorIdentification":"organisationIdentification",
  "typ": "JOSE"}

    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "content-type",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "content-type",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    }

    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,undefined,"taxRequirement")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignaturePOSTpaymentstaxRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "content-type",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "content-type",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,undefined,"taxRequirement")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignaturePATCHpaymentConsentstaxRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "content-type",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "content-type",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,undefined,"taxRequirement")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureGETpaymentConsentstaxRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,"taxRequirement")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureGETpaymentConsentstaxRequirementDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,"taxRequirement")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureGETpaymentstaxRequirementDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,"taxRequirement")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureDELETEpaymentstaxRequirementDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,"taxRequirement")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureGETpaymentstaxRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,"taxRequirement")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureDELETEpaymentstaxRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,"taxRequirement")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}


async function generateSignatureDELETEpaymentConsentstaxRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,"taxRequirement")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureDELETEpaymentConsentstaxRequirementDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,"taxRequirement")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignaturePUTcreateExternalRepresentationtaxRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,"taxRequirement")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignaturePUTcreateExternalRepresentationSpecialParttaxRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId,"taxRequirement")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}


async function createPaymentConsentstaxRequirement(config,access_token){
    let requestBodyName = "POSTpaymentConsents/taxRequirements1.2"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("taxRequirement",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "POSTpaymentConsentsTaxRequirement"
    let signature = await generateSignaturePOSTpaymentConsentstaxRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/taxRequirement", {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "POST",
    mode: "cors",
    headers: headersList,
    body: JSON.stringify(paf.sortObjectAlphabeticallyNew(paf.getRequestBody("taxRequirement",requestBodyName)))
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: POST \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("taxRequirement",requestBodyName)))
  return response.json();
}

async function postPaymentstaxRequirement(config,access_token){
    let requestBodyName = "POSTpaymentsTaxRequirement1"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("taxRequirement",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "POSTpaymentsTaxRequirement"
    let signature = await generateSignaturePOSTpaymentstaxRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/taxRequirement", {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "POST",
    mode: "cors",
    headers: headersList,
    body: JSON.stringify(paf.sortObjectAlphabeticallyNew(paf.getRequestBody("taxRequirement",requestBodyName)))
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: POST \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("taxRequirement",requestBodyName)))
  return response.json();
}

async function generateSpecParttaxRequirement(config,requestBodyWithExternalRepresentation){
    let imitIns = await paf.createImitationInsert(config,requestBodyWithExternalRepresentation)
    let specPart = await createSpecialPartObjecttaxRequirement(config,imitIns.ResultB64)
    console.log("specialPart \n" + JSON.stringify(specPart))
    setRequestBody("taxRequirement","PUTpaymentConsentsCreateExternalRepresentationSpecialPartTaxRequirement",specPart)
    let externalRepresentationSpecialPart = await createExternalRepresentationSpecialParttaxRequirement(config,specPart)
    specPart.specialPart.externalRepresentationSpecialPart = externalRepresentationSpecialPart.data.externalRepresentationSpecialPart
    return specPart
}

async function createSpecialPartObjecttaxRequirement(config,imitIns){
  const currentDate = new Date();
  const formattedDate = format(currentDate, "yyyy-MM-dd'T'HH:mm:ssXXX");
  currentDate.setMinutes(currentDate.getMinutes()+2)
  const updatedFormattedDate = format(currentDate, "yyyy-MM-dd'T'HH:mm:ssXXX");
  return {
    "specialPart":
        {
          "HMAC": imitIns,
          "OTP": config.client_otp,
          "OTPdateTime": formattedDate,
          "mobileNumber": config.mobile_number,
          "signatureDateTime": updatedFormattedDate,
          "status": "Authorised",
          "statusUpdateDateTime": formattedDate,
          "subjectKeyIdentifier": "B6D7498EE0E67A368E921810E53849B0C7A69646"
        }
  }
}
async function createExternalRepresentationtaxRequirement(config, requestBody){
    await paf.appendToDefinedFile("logs.txt","EXTERNAL REPRESENTATION","")
    let token = await paf.createDboClientToken(config)
    let access_token = token.access_token

    let requestBodyName = "PUTpaymentConsentsCreateExternalRepresentationTaxRequirement"
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody = {
        "Auth":{
            "CryptoType":1,
        },
        "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64

    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "PUTpaymentConsentsCreateExternalRepresentationTaxRequirement"
    let signature = await generateSignaturePUTcreateExternalRepresentationtaxRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,["NoIdempotencyKey"])
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))

    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":"
    }
    console.log(JSON.stringify(headersList))
    await paf.appendToDefinedFile("logs.txt","fullRequest","method: PUT \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("taxRequirement",requestBodyName)))
  // const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/taxRequirement/createExternalRepresentation", {
  //   method: "PUT",
  //   mode: "cors",
  //   headers: headersList,
  //   body: JSON.stringify(requestBody)
  // });
  let response = {"data":{"externalRepresentation":""}}
  // return response.json();
  return response
}

async function createExternalRepresentationSpecialParttaxRequirement(config, requestBody){
    await paf.appendToDefinedFile("logs.txt","EXTERNAL REPRESENTATION SPECIAL PART","")
    let token = await paf.createDboClientToken(config)
    let access_token = token.access_token

    let requestBodyName = "PUTpaymentConsentsCreateExternalRepresentationSpecialPartTaxRequirement"
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody = {
        "Auth":{
            "CryptoType":1,
        },
        "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64

    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "PUTpaymentConsentsExternalRepresentationSpecialPartTaxRequirement"
    let signature = await generateSignaturePUTcreateExternalRepresentationSpecialParttaxRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,["NoIdempotencyKey"])
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))

    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    console.log(JSON.stringify(headersList))
  //   const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/createSpecialPartExternalRepresentation", {
  //   method: "PUT",
  //   mode: "cors",
  //   headers: headersList,
  //   body:JSON.stringify(requestBody)
  // });
    let response = {"data":{"externalRepresentationSpecialPart":""}}
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: POST \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("taxRequirement",requestBodyName)))
  // return response.json();
    return response
}

async function generatePaymentConsenttaxRequirementAuthoriseBody(config,accList,createPaymentConsentstaxRequirementResponse){
    delete accList.data.account
    delete accList.data.initiation.enclosedFile
    delete accList.data.initiation.listPassportData
    delete accList.data.initiation.listAccounts
    delete accList.data.charge
    // accList.data.statusUpdateDateTime = accList.data.statusUpdateDateTime.split('+')[0]
    // accList.data.creationDateTime = accList.data.creationDateTime.split('+')[0]
    let sortedAccList = paf.sortObjectAlphabeticallyNew(accList)
    let requestBody = sortedAccList
    setRequestBody("taxRequirement","PUTpaymentConsentsCreateExternalRepresentationTaxRequirement",requestBody)
    let latestAuthType = authType;
    authType = "OBclientCredentials"
    let externalRepresentation = await createExternalRepresentationtaxRequirement(config, requestBody)
    authType = latestAuthType
    requestBody.data.externalRepresentation = externalRepresentation.data.externalRepresentation
    requestBody = paf.sortObjectAlphabeticallyNew(requestBody)

    latestAuthType = authType;
    authType = "OBclientCredentials"
    let specPart = await generateSpecParttaxRequirement(config, requestBody)
    authType = latestAuthType
    requestBody.specialPart = specPart.specialPart
    requestBody = paf.renameKeyInObject(requestBody,"paymentConsentId","taxRequirementConsentId")
    requestBody = paf.sortObjectAlphabeticallyNew(requestBody)

    return requestBody
}

async function patchPaymentConsenttaxRequirement(config,access_token,accList,createPaymentConsentstaxRequirementResponse){
    let requestBodyName = "PATCHpaymentConsents/taxRequirement1"
    let requestBody = await generatePaymentConsenttaxRequirementAuthoriseBody(config,accList,createPaymentConsentstaxRequirementResponse)
    setRequestBody("taxRequirement",requestBodyName,requestBody)
    let savedBody = paf.getRequestBody("taxRequirement",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "PATCHpaymentConsentsTaxRequirement"
    let signature = await generateSignaturePATCHpaymentConsentstaxRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/taxRequirement", {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "PATCH",
    mode: "cors",
    headers: headersList,
    body: JSON.stringify(paf.sortObjectAlphabeticallyNew(paf.getRequestBody("taxRequirement",requestBodyName)))
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: PATCH \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("taxRequirement",requestBodyName)))
  return response.json();
}

async function getPaymentConsentstaxRequirement(config,access_token,taxRequirementConsentId){
    let requestBodyName = "GETpaymentConsents/taxRequirement"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("taxRequirement",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentConsentsTaxRequirement"
    let signature = await generateSignatureGETpaymentConsentstaxRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/taxRequirement/" + taxRequirementConsentId, {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("taxRequirement",requestBodyName)))
  return response.json();
}

async function getPaymentConsentstaxRequirementDBO(config,access_token,taxRequirementConsentId){
    let latestAuthType = authType
    let requestBodyName = "GETpaymentConsents/taxRequirementDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("taxRequirement",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    authType = "OBclientCredentials"
    paf.setAuthType("OBclientCredentials")
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentConsentsTaxRequirementDBO"
    let signature = await generateSignatureGETpaymentConsentstaxRequirementDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/taxRequirement/" + taxRequirementConsentId +"/PSUorPAU/V087_TEST1", {
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("taxRequirement",requestBodyName)))
  authType = latestAuthType
  paf.setAuthType(latestAuthType)
  return response.json();
}

async function getPaymentstaxRequirementDBO(config,access_token,taxRequirementId){
    let latestAuthType = authType
    let requestBodyName = "GETpayments/taxRequirementDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("taxRequirement",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    authType = "OBclientCredentials"
    paf.setAuthType("OBclientCredentials")
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentstaxRequirementDBO"
    let signature = await generateSignatureGETpaymentstaxRequirementDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/taxRequirement/" + taxRequirementId +"/PSUorPAU/V087_TEST1", {
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("taxRequirement",requestBodyName)))
  authType = latestAuthType
  paf.setAuthType(latestAuthType)
  return response.json();
}

async function deletePaymentstaxRequirementDBO(config,access_token,taxRequirementId){
    let latestAuthType = authType
    let requestBodyName = "DELETEpayments/taxRequirementDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("taxRequirement",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    authType = "OBclientCredentials"
    paf.setAuthType("OBclientCredentials")
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentstaxRequirementDBO"
    let signature = await generateSignatureDELETEpaymentstaxRequirementDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/taxRequirement/" + taxRequirementId +"/PSUorPAU/V087_TEST1", {
    method: "DELETE",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("taxRequirement",requestBodyName)))
  authType = latestAuthType
  paf.setAuthType(latestAuthType)
  return response.json();
}

async function getPaymentstaxRequirement(config,access_token,taxRequirementId){
    let requestBodyName = "GETpayments/taxRequirement"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("taxRequirement",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentstaxRequirement"
    let signature = await generateSignatureGETpaymentstaxRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/taxRequirement/" + taxRequirementId, {
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("taxRequirement",requestBodyName)))
  return response.json();
}

async function deletePaymentstaxRequirement(config,access_token,taxRequirementId){
    let requestBodyName = "DELETEpayments/taxRequirement"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("taxRequirement",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentstaxRequirement"
    let signature = await generateSignatureDELETEpaymentstaxRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/taxRequirement/" + taxRequirementId, {
    method: "DELETE",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("taxRequirement",requestBodyName)))
  return response.json();
}

async function deletePaymentConsentstaxRequirement(config,access_token,taxRequirementConsentId){
    let requestBodyName = "DELETEpaymentConsentstaxRequirement"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("taxRequirement",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentConsentstaxRequirement"
    let signature = await generateSignatureDELETEpaymentConsentstaxRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
      let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/taxRequirement/" + taxRequirementConsentId, {
    method: "DELETE",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: DELETE \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("taxRequirement",requestBodyName)))
  // return response.json();
}

async function deletePaymentConsentstaxRequirementDBO(config,access_token,taxRequirementConsentId){
    let prevAuthType = authType
    authType = "OBclientCredentials"
    setAuthType("OBclientCredentials")
    let requestBodyName = "DELETEpaymentConsentstaxRequirementDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("taxRequirement",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentConsentstaxRequirementDBO"
    let signature = await generateSignatureDELETEpaymentConsentstaxRequirementDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,taxRequirementConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
      let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/taxRequirement/" + taxRequirementConsentId + "/PSUorPAU/V087_TEST1", {
    method: "DELETE",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: DELETE \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("taxRequirement",requestBodyName)))
  authType = prevAuthType
  setAuthType(prevAuthType)
  // return response.json();
}

async function modifyPATCHpaymentConsenttaxRequirementBody(endToEndIdentification,instructionIdentification){
    let body = await paf.getRequestBody("tpeAuth","PATCHpaymentInstant/invoice1")
    body.data.modification.paymentEndToEndIdentification = endToEndIdentification
    body.data.modification.paymentInstructionIdentification = instructionIdentification
    await paf.appendToDefinedFile("logs.txt","patchInstantInvoice_modifiedBody",JSON.stringify(body))
    await paf.setRequestBody("tpeAuth","PATCHpaymentInstant/invoice1",body)
}

async function modifyPostPaymentstaxRequirementBody(taxRequirementConsentId,initiation){
    let body = await paf.getRequestBody("taxRequirement","POSTpaymentsTaxRequirement1")
    body.data.taxRequirementConsentId = taxRequirementConsentId
    body.data.initiation = initiation
    await paf.appendToDefinedFile("logs.txt","POSTpaymentsTaxRequirement1_modifiedBody",JSON.stringify(body))
    await setRequestBody("taxRequirement","POSTpaymentsTaxRequirement1",body)
}

async function main1() {
    const config = {
        alg: "BELTM256",
        typ: "JOSE",
        url_kc: "https://sc-map-testversion-vip.softclub.by:7891/",
        url_swagger: "https://sc-map-testversion-vip.softclub.by:8008/",
        client_id_pisp: "PISP2TEST",
        client_secret_pisp: "Cgxb4O9UWS4HZwrpbf3bfefdrZTStubt",
        client_id_qpisp: "BELKARTPAY_NPC_TEST",
        client_secret_qpisp: "aES5biV0eWVkVWUHzD36it5X2yE7DSkF",
        client_id_tpe: "ENTERPRISESOFT",
        client_secret_tpe: "Nisll6ytlAAtYGqb7W1Kus539rfLAZuP",
        client_id_dbo: "digitalChannels",
        client_secret_dbo: "rvDMLEf5Njz6L5BGpst4dLP1hMrBWxEV",
        // client_id_dbo:"iBankASB",
        // client_secret_dbo:"zoVP6g4yTFsW64a6hAbSMe43mQrWxmJl",
        apikey: "dcbeebf6-1d34-4bb0-82cf-bcfe185e037f", //V087_TEST1
        // apikey: "ed999501-fe4e-4f18-845e-d69eab692941", //test-client.12
        client_otp: "asb123",
        mobile_number: "+375-255427989"
    }

    // const config = {
    //     alg: "BELTM256",
    //     typ: "JOSE",
    //     url_kc: "https://open-banking-akbb.softclub.by/",
    //     url_swagger: "https://open-banking-akbb.softclub.by/",
    //     // url_swagger:"192.168.166.122:10081/",
    //     client_id_pisp: "PISPTEST",
    //     client_secret_pisp: "P9XzSwdBkRw17nu1V2CSM3eogszWC0XQ",
    //     client_id_qpisp: "BELKARTPAY_NPC_TEST",
    //     client_secret_qpisp: "veZxbezDPxbOw4l0RIzfCO7TDNALvHUV",
    //     client_id_dbo: "digitalChannelsNew",
    //     client_secret_dbo: "mnoSdqcDruzrrBqev06ZLkHOGE3xRayy",
    //     client_id_tpe: "ENTERPRISESOFT",
    //     client_secret_tpe: "VUpshCxrbwRlCbTFDFpQhOUh78zeUPlU",
    //     apikey: "0788319d-048e-449b-b785-b0e740ddc5ac", //V093_TEST1
    //     // apikey: "5bcda088-5cdb-4f98-8cc6-b1c42eb19c39", //V087_TEST1
    //     client_otp: "asb123",
    //     mobile_number: "+375-255427989"
    // }
     let pispAccessToken
    let pispToken = await paf.createTokenPISP(config)
    pispAccessToken = pispToken.access_token
    await paf.appendToDefinedFile("logs.txt","PISPtoken",pispAccessToken.toString())
    console.log(pispToken)
    console.log("pisp_access_token : "+pispAccessToken)
    let dboClientAccessToken
    let dboClientToken = await paf.createDboClientToken(config)
    dboClientAccessToken = dboClientToken.access_token

    let createPaymentConsentsTaxRequirementResponse = await createPaymentConsentstaxRequirement(config,pispAccessToken)
    // let createPaymentConsentstaxRequirementResponse = {"data":{"charge":[],"creationDateTime":"2025-10-20T19:19:23.514437627+03:00","initiation":{"amount":"201.00","creditor":{"countryOfResidence":"BY","name":"ST-”KRYSHKOVSKAYA short“ короткое наименование","organisationIdentification":[{"code":"TXID","identification":"INN101103846"}],"privateIdentification":[]},"creditorAccount":{"identification":"BY48AKBB30121272200458000933","schemeName":"BY.NBRB.IBAN"},"creditorAgent":{"identification":"AKBBBY2X","name":"ОАО 'АСБ Беларусбанк'"},"currency":"BYN","endToEndIdentification":"01.20250412.8bd17819bcf8d8da","instructionIdentification":"795SDBO20250412E5AE1D70F25F431F84DD","taxRequirement":[{"IBAN":"BY21AKBB30140012426250070000","amount":"100.00","name":"Белуга Митрофан Доромидонтович"},{"IBAN":"BY06AKBB30140012426260070000","amount":"101.00","name":"Килька Авдотья Никитишна"}],"localInstrument":"BY.NBRB.BISS.NORMAL","requestedExecutionDate":"2025-12-12"},"link":"https://dc.asb.by/paymentСonsents/taxRequirement/15582e04-8249-4957a675-0ba7c95535fb","taxRequirementConsentId":"15582e04-8249-4957a675-0ba7c95535fb","status":"AwaitingAuthorisation","statusUpdateDateTime":"2025-10-20T19:19:23.514437627+03:00"},"links":{"self":"https://sc-map-testversion-vip.softclub.by:8008/oapi-channel/open-banking/v1.0/paymentConsents/taxRequirement/15582e04-8249-4957a675-0ba7c95535fb"},"meta":{"totalPages":1}}
    console.log(JSON.stringify(createPaymentConsentsTaxRequirementResponse))
    await paf.appendToDefinedFile("logs.txt","createPaymentConsentstaxRequirementResponse",JSON.stringify(createPaymentConsentsTaxRequirementResponse))

    let getPaymentConsentstaxRequirementResponse = await getPaymentConsentstaxRequirement(config,pispAccessToken,createPaymentConsentsTaxRequirementResponse.data.taxRequirementConsentId)
    console.log(JSON.stringify(getPaymentConsentstaxRequirementResponse))
    await paf.appendToDefinedFile("logs.txt","getPaymentConsentstaxRequirementResponse",JSON.stringify(getPaymentConsentstaxRequirementResponse))

    let getPaymentConsentstaxRequirementDBOResponse = await getPaymentConsentstaxRequirementDBO(config,dboClientAccessToken,createPaymentConsentsTaxRequirementResponse.data.taxRequirementConsentId)
    console.log(JSON.stringify(getPaymentConsentstaxRequirementDBOResponse))
    await paf.appendToDefinedFile("logs.txt","getPaymentConsentstaxRequirementDBOResponse",JSON.stringify(getPaymentConsentstaxRequirementDBOResponse))

    // let deletePaymentConsentstaxRequirementResponse = await deletePaymentConsentstaxRequirement(config,pispAccessToken,createPaymentConsentstaxRequirementResponse.data.taxRequirementConsentId)
    // let deletePaymentConsentstaxRequirementDBOResponse = await deletePaymentConsentstaxRequirementDBO(config,dboClientAccessToken,createPaymentConsentstaxRequirementResponse.data.taxRequirementConsentId)

    // getPaymentConsentstaxRequirementResponse = await getPaymentConsentstaxRequirement(config,pispAccessToken,createPaymentConsentstaxRequirementResponse.data.taxRequirementConsentId)
    // console.log(JSON.stringify(getPaymentConsentstaxRequirementResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentConsentstaxRequirementResponse",JSON.stringify(getPaymentConsentstaxRequirementResponse))

    let accListPaymentstaxRequirement = await paf.getAccListPayments(config,dboClientAccessToken,createPaymentConsentsTaxRequirementResponse.data.taxRequirementConsentId)
    console.log(JSON.stringify(accListPaymentstaxRequirement))
    await paf.appendToDefinedFile("logs.txt","accListPaymentstaxRequirement",JSON.stringify(accListPaymentstaxRequirement))


    let patchPaymentConsenttaxRequirementResponse = await patchPaymentConsenttaxRequirement(config,dboClientAccessToken,accListPaymentstaxRequirement,createPaymentConsentsTaxRequirementResponse)
    console.log(JSON.stringify(patchPaymentConsenttaxRequirementResponse))
    await paf.appendToDefinedFile("logs.txt","patchPaymentConsenttaxRequirementResponse",JSON.stringify(patchPaymentConsenttaxRequirementResponse))

    await modifyPostPaymentstaxRequirementBody(createPaymentConsentsTaxRequirementResponse.data.taxRequirementConsentId,createPaymentConsentsTaxRequirementResponse.data.initiation)
    let postPaymentstaxRequirementResponse = await postPaymentstaxRequirement(config,pispAccessToken)
    console.log(JSON.stringify(postPaymentstaxRequirementResponse))
    await paf.appendToDefinedFile("logs.txt","createPaymentstaxRequirementResponse",JSON.stringify(postPaymentstaxRequirementResponse))

    let taxRequirementId = postPaymentstaxRequirementResponse.data.taxRequirementId
    let getPaymentstaxRequirementDBOResponse = await getPaymentstaxRequirementDBO(config,dboClientAccessToken,taxRequirementId)
    console.log(JSON.stringify(getPaymentstaxRequirementDBOResponse))
    await paf.appendToDefinedFile("logs.txt","getPaymenttaxRequirementDBOResponse",JSON.stringify(getPaymentstaxRequirementDBOResponse))

    let getPaymentstaxRequirementResponse = await getPaymentstaxRequirement(config,dboClientAccessToken,taxRequirementId)
    console.log(JSON.stringify(getPaymentstaxRequirementResponse))
    await paf.appendToDefinedFile("logs.txt","getPaymenttaxRequirementResponse",JSON.stringify(getPaymentstaxRequirementResponse))

    // await deletePaymentstaxRequirementDBO(config,dboClientAccessToken,taxRequirementId)
    // console.log(JSON.stringify(deletePaymentstaxRequirementDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","deletePaymenttaxRequirementDBOResponse",JSON.stringify(deletePaymentstaxRequirementDBOResponse))

    // getPaymentstaxRequirementDBOResponse = await getPaymentstaxRequirementDBO(config,dboClientAccessToken,taxRequirementId)
    // console.log(JSON.stringify(getPaymentstaxRequirementDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymenttaxRequirementDBOResponse",JSON.stringify(getPaymentstaxRequirementDBOResponse))

    // await deletePaymentstaxRequirement(config,dboClientAccessToken,taxRequirementId)
    // console.log(JSON.stringify(deletePaymentstaxRequirementResponse))
    // await paf.appendToDefinedFile("logs.txt","deletePaymenttaxRequirementResponse",JSON.stringify(deletePaymentstaxRequirementResponse))

    // let getPaymentstaxRequirementDBOResponse = await getPaymentstaxRequirementDBO(config,dboClientAccessToken,taxRequirementId)
    // console.log(JSON.stringify(getPaymentstaxRequirementDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymenttaxRequirementDBOResponse",JSON.stringify(getPaymentstaxRequirementDBOResponse))
}

main1()