const uuid = require("uuid");
const paf = require("./PISPauth")
const {setRequestBody} = require("./requestBodies");
const {format} = require("date-fns");
const {setAuthType} = require("./PISPauth");

class UnixDate{
    unixTimestamp = 0
    constructor(){
        this.unixTimestamp = Math.floor(Date.now() / 1000);
    }

    getCurrentDate(){
        return this.unixTimestamp
    }
    getNextDayDate(){
        return this.unixTimestamp + 3600
    }

    getDateNsecondsAgo(n){
        return this.unixTimestamp  - n
    }

    getFormattedDate(unixTime) {
        // Преобразуем Unix-время в миллисекунды (умножаем на 1000)
        const date = new Date(unixTime * 1000);
        // Используем метод toUTCString() для форматирования даты
        return date.toUTCString();
    }

    getISOWithTimeZone(unixTime, timeZoneOffset = '+03:00') {
        // Преобразуем Unix-время в миллисекунды
        const date = new Date(unixTime  * 1000 );

        // Получаем компоненты даты
        const year = date.getUTCFullYear();
        const month = String(date.getUTCMonth() + 1).padStart(2, '0'); // Месяцы начинаются с 0
        const day = String(date.getUTCDate()).padStart(2, '0');
        const hours = String(date.getUTCHours()).padStart(2, '0');
        const minutes = String(date.getUTCMinutes()).padStart(2, '0');
        const seconds = String(date.getUTCSeconds()).padStart(2, '0');

        // Формируем строку в формате ISO 8601 с временной зоной
        return `${year}-${month}-${day}T${hours}:${minutes}:${seconds}${timeZoneOffset}`;
    }

    getShortedDate(unixTime){
        const date = new Date(unixTime  * 1000 );

        const year = date.getUTCFullYear();
        const month = String(date.getUTCMonth() + 1).padStart(2, '0'); // Месяцы начинаются с 0
        const day = String(date.getUTCDate()).padStart(2, '0');

        return `${year}${month}${day}`;
    }

}

let unixDate = new UnixDate()
const currentDate = new Date();

let startTime = ""
let authType = "PAUapikey" // PAUapikey, OBclientCredentials
paf.setAuthType("PAUapikey")
process.env["NODE_TLS_REJECT_UNAUTHORIZED"] = 0;

async function generateSignaturePOSTpaymentConsentsVRP(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "content-type",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "content-type",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,undefined,"VRP")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignaturePOSTpaymentsVRP(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "content-type",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "content-type",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,undefined,"VRP")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignaturePATCHpaymentConsentsVRP(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "content-type",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "content-type",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,undefined,"VRP")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureGETpaymentConsentsVRP(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,"VRP")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureGETpaymentConsentsVRPDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,"VRP")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureGETpaymentsVRPDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,"VRP")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureDELETEpaymentsVRPDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,"VRP")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureGETpaymentsVRP(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,"VRP")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureDELETEpaymentsVRP(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,"VRP")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}


async function generateSignatureDELETEpaymentConsentsVRP(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,"VRP")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureDELETEpaymentConsentsVRPDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,"VRP")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignaturePUTcreateExternalRepresentationVRP(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "content-type",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "content-type",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,"VRP")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignaturePUTcreateExternalRepresentationSpecialPartVRP(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId,"VRP")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}


async function createpaymentConsentsVRP(config,access_token){
    let requestBodyName = "POSTpaymentConsents/VRP1"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("VRP",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "POSTpaymentConsentsVRP"
    let signature = await generateSignaturePOSTpaymentConsentsVRP(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/VRP", {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "POST",
    mode: "cors",
    headers: headersList,
    body: JSON.stringify(paf.sortObjectAlphabeticallyNew(paf.getRequestBody("VRP",requestBodyName)))
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: POST \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("VRP",requestBodyName)))
  return response.json();
}

async function postPaymentsVRP(config,access_token){
    let requestBodyName = "POSTpaymentsVRP1"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("VRP",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "POSTpaymentsVRP"
    let signature = await generateSignaturePOSTpaymentsVRP(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/VRP", {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "POST",
    mode: "cors",
    headers: headersList,
    body: JSON.stringify(paf.sortObjectAlphabeticallyNew(paf.getRequestBody("VRP",requestBodyName)))
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: POST \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("VRP",requestBodyName)))
  return response.json();
}

async function generateSpecPartVRP(config,requestBodyWithExternalRepresentation){
    let imitIns = await paf.createImitationInsert(config,requestBodyWithExternalRepresentation)
    let specPart = await createSpecialPartObjectVRP(config,imitIns.ResultB64)
    console.log("specialPart \n" + JSON.stringify(specPart))
    setRequestBody("VRP","PUTpaymentConsentsCreateExternalRepresentationSpecialPartVRP",specPart)
    let externalRepresentationSpecialPart = await createExternalRepresentationSpecialPartVRP(config,specPart)
    specPart.specialPart.externalRepresentationSpecialPart = externalRepresentationSpecialPart.data.externalRepresentationSpecialPart
    return specPart
}

async function createSpecialPartObjectVRP(config,imitIns){
  const currentDate = new Date();
  const formattedDate = format(currentDate, "yyyy-MM-dd'T'HH:mm:ssXXX");
  currentDate.setMinutes(currentDate.getMinutes()+2)
  const updatedFormattedDate = format(currentDate, "yyyy-MM-dd'T'HH:mm:ssXXX");
  return {
    "specialPart":
        {
          "HMAC": imitIns,
          "OTP": config.client_otp,
          "OTPdateTime": formattedDate,
          "mobileNumber": config.mobile_number,
          "signatureDateTime": updatedFormattedDate,
          "status": "Authorised",
          "statusUpdateDateTime": formattedDate,
          "subjectKeyIdentifier": "B6D7498EE0E67A368E921810E53849B0C7A69646"
        }
  }
}
async function createExternalRepresentationVRP(config, requestBody){
    await paf.appendToDefinedFile("logs.txt","EXTERNAL REPRESENTATION","")
    let token = await paf.createDboClientToken(config)
    let access_token = token.access_token

    let requestBodyName = "PUTpaymentConsentsCreateExternalRepresentationVRP"
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody = {
        "Auth":{
            "CryptoType":1,
        },
        "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64

    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "PUTpaymentConsentsCreateExternalRepresentationVRP"
    let signature = await generateSignaturePUTcreateExternalRepresentationVRP(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,["NoIdempotencyKey"])
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))

    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":"
    }
    console.log(JSON.stringify(headersList))
    await paf.appendToDefinedFile("logs.txt","fullRequest","method: PUT \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("VRP",requestBodyName)))
  const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/VRP/createExternalRepresentation", {
    method: "PUT",
    mode: "cors",
    headers: headersList,
    body: JSON.stringify(requestBody)
  });
  // let response = {"data":{"externalRepresentation":""}}
  return response.json();
  // return response
}

async function createExternalRepresentationSpecialPartVRP(config, requestBody){
    await paf.appendToDefinedFile("logs.txt","EXTERNAL REPRESENTATION SPECIAL PART","")
    let token = await paf.createDboClientToken(config)
    let access_token = token.access_token

    let requestBodyName = "PUTpaymentConsentsCreateExternalRepresentationSpecialPartVRP"
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody = {
        "Auth":{
            "CryptoType":1,
        },
        "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64

    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "PUTpaymentConsentsExternalRepresentationSpecialPartVRP"
    let signature = await generateSignaturePUTcreateExternalRepresentationSpecialPartVRP(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,["NoIdempotencyKey"])
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))

    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    console.log(JSON.stringify(headersList))
  //   const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/createSpecialPartExternalRepresentation", {
  //   method: "PUT",
  //   mode: "cors",
  //   headers: headersList,
  //   body:JSON.stringify(requestBody)
  // });
    let response = {"data":{"externalRepresentationSpecialPart":""}}
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: POST \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("VRP",requestBodyName)))
  // return response.json();
    return response
}

async function generatePaymentConsentVRPAuthoriseBody(config,accList){
    delete accList.data.account
    delete accList.data.initiation.enclosedFile
    delete accList.data.initiation.listAccounts
    delete accList.data.initiation.listPassportData
    delete accList.data.initiation.regulatoryReporting
    delete accList.data.charge
    accList.data
    // accList.data.statusUpdateDateTime = accList.data.statusUpdateDateTime.split('+')[0]
    // accList.data.creationDateTime = accList.data.creationDateTime.split('+')[0]
    let sortedAccList = paf.sortObjectAlphabeticallyNew(accList)
    let requestBody = sortedAccList
    requestBody = paf.renameKeyInObject(requestBody,"paymentConsentId","VRPConsentId")
    requestBody = paf.sortObjectAlphabeticallyNew(requestBody)
    setRequestBody("VRP","PUTpaymentConsentsCreateExternalRepresentationVRP",requestBody)
    paf.setAuthType("OBclientCredentials")
    let externalRepresentation = await createExternalRepresentationVRP(config, requestBody)
    requestBody.data.externalRepresentation = externalRepresentation.data.externalRepresentation
    requestBody = paf.sortObjectAlphabeticallyNew(requestBody)

    let specPart = await generateSpecPartVRP(config, requestBody)
    requestBody.specialPart = specPart.specialPart
    requestBody = paf.sortObjectAlphabeticallyNew(requestBody)
    return requestBody
}

async function patchPaymentConsentVRP(config,access_token,accList){
    let latestAuthType = authType
    authType = "OBclientCredentials"
    setAuthType("OBclientCredentials")
    let requestBodyName = "PATCHpaymentConsents/VRP1"
    let requestBody = await generatePaymentConsentVRPAuthoriseBody(config,accList)
    setRequestBody("VRP",requestBodyName,requestBody)
    let savedBody = paf.getRequestBody("VRP",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "PATCHpaymentConsentsVRP"
    let signature = await generateSignaturePATCHpaymentConsentsVRP(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    startTime = performance.now()
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/VRP", {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "PATCH",
    mode: "cors",
    headers: headersList,
    body: JSON.stringify(paf.sortObjectAlphabeticallyNew(paf.getRequestBody("VRP",requestBodyName)))
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: PATCH \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("VRP",requestBodyName)))
  authType = latestAuthType
  setAuthType(latestAuthType)
  return response.json();
}

async function getpaymentConsentsVRP(config,access_token,VRPConsentId){
    let requestBodyName = "GETpaymentConsents/VRP"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("VRP",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentConsentsVRP"
    let signature = await generateSignatureGETpaymentConsentsVRP(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/VRP/" + VRPConsentId, {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("VRP",requestBodyName)))
  return response.json();
}

async function getpaymentConsentsVRPDBO(config,access_token,VRPConsentId){
    let latestAuthType = authType
    let requestBodyName = "GETpaymentConsents/VRPDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("VRP",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    authType = "OBclientCredentials"
    paf.setAuthType("OBclientCredentials")
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentConsentsVRPDBO"
    let signature = await generateSignatureGETpaymentConsentsVRPDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/VRP/" + VRPConsentId +"/PSUorPAU/V087_TEST1", {
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("VRP",requestBodyName)))
  authType = latestAuthType
  paf.setAuthType(latestAuthType)
  return response.json();
}

async function getPaymentsVRPDBO(config,access_token,VRPId){
    let latestAuthType = authType
    let requestBodyName = "GETpayments/VRPDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("VRP",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    authType = "OBclientCredentials"
    paf.setAuthType("OBclientCredentials")
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentsVRPDBO"
    let signature = await generateSignatureGETpaymentsVRPDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,VRPId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/VRP/" + VRPId +"/PSUorPAU/V087_TEST1", {
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("VRP",requestBodyName)))
  authType = latestAuthType
  paf.setAuthType(latestAuthType)
  return response.json();
}

async function deletePaymentsVRPDBO(config,access_token,VRPId){
    let latestAuthType = authType
    let requestBodyName = "DELETEpayments/VRPDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("VRP",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    authType = "OBclientCredentials"
    paf.setAuthType("OBclientCredentials")
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentsVRPDBO"
    let signature = await generateSignatureDELETEpaymentsVRPDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,VRPId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/VRP/" + VRPId +"/PSUorPAU/V087_TEST1", {
    method: "DELETE",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("VRP",requestBodyName)))
  authType = latestAuthType
  paf.setAuthType(latestAuthType)
  return response.json();
}

async function getPaymentsVRP(config,access_token,VRPId){
    let requestBodyName = "GETpayments/VRP"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("VRP",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentsVRP"
    let signature = await generateSignatureGETpaymentsVRP(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,VRPId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/VRP/" + VRPId, {
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("VRP",requestBodyName)))
  return response.json();
}

async function deletePaymentsVRP(config,access_token,VRPId){
    let requestBodyName = "DELETEpayments/VRP"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("VRP",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentsVRP"
    let signature = await generateSignatureDELETEpaymentsVRP(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,VRPId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/VRP/" + VRPId, {
    method: "DELETE",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("VRP",requestBodyName)))
  return response.json();
}

async function deletepaymentConsentsVRP(config,access_token,VRPConsentId){
    let requestBodyName = "DELETEpaymentConsentsVRP"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("VRP",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentConsentsVRP"
    let signature = await generateSignatureDELETEpaymentConsentsVRP(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
      let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/VRP/" + VRPConsentId, {
    method: "DELETE",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: DELETE \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("VRP",requestBodyName)))
  // return response.json();
}

async function deletepaymentConsentsVRPDBO(config,access_token,VRPConsentId){
    let prevAuthType = authType
    authType = "OBclientCredentials"
    setAuthType("OBclientCredentials")
    let requestBodyName = "DELETEpaymentConsentsVRPDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("VRP",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentConsentsVRPDBO"
    let signature = await generateSignatureDELETEpaymentConsentsVRPDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,VRPConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
      let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    // const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/VRP/" + VRPConsentId + "/PSUorPAU/V087_TEST1", {
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/VRP/" + VRPConsentId + "/PSUorPAU/V087_TEST1", {
    method: "DELETE",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: DELETE \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("VRP",requestBodyName)))
  authType = prevAuthType
  setAuthType(prevAuthType)
  // return response.json();
}

async function modifyPATCHpaymentConsentVRPBody(uri_paymentLink,instantConsentId,invoiceInstantId,instructionIdentification,endToEndIdentification){
    let body = await paf.getRequestBody("tpeAuth","PATCHpaymentInstant/invoice1")
    body.data.paymentLink[0]["URI"] = uri_paymentLink
    body.data.instantConsentId = instantConsentId
    body.data.invoiceInstantId = invoiceInstantId
    body.data.modification.paymentEndToEndIdentification = endToEndIdentification
    body.data.modification.paymentInstructionIdentification = instructionIdentification
    await paf.appendToDefinedFile("logs.txt","patchInstantInvoice_modifiedBody",JSON.stringify(body))
    await paf.setRequestBody("tpeAuth","PATCHpaymentInstant/invoice1",body)
}

async function modifyPostPaymentsVRPBody(VRPConsentId,initiation,risk){
    let body = await paf.getRequestBody("VRP","POSTpaymentsVRP1")
    let localAmount = initiation.amount ? initiation.amount: "1.00"
    body.data.VRPConsentId = VRPConsentId
    body.data.initiation = initiation
    // "creditor":localCreditor,"creditorAccount":localCreditorAccount,"creditorAgent":localCreditorAgent
    body.data.instruction = {"instructionIdentification":"795SDBO" + unixDate.getShortedDate(unixDate.getCurrentDate()) + paf.generateRandomHex(16),"endToEndIdentification":"01." + unixDate.getShortedDate(unixDate.getCurrentDate()) + "." + paf.generateRandomHex(16),"localInstrument":initiation.localInstrument,"amount":localAmount, "currency":initiation.currency,"remittanceInformation":initiation.remittanceInformation}
    if(initiation.creditor){
        body.data.instruction.creditor = initiation.creditor
    }
    if(initiation.creditorAccount){
        body.data.instruction.creditorAccount = initiation.creditorAccount
    }
    if(initiation.creditorAgent){
        body.data.instruction.creditorAgent = initiation.creditorAgent
    }
    body.risk = risk
    await paf.appendToDefinedFile("logs.txt","POSTpaymentsVRP1_modifiedBody",JSON.stringify(body))
    await setRequestBody("VRP","POSTpaymentsVRP1",body)
}

async function modifyPostpaymentConsentsVRPBody(){
    let body = await paf.getRequestBody("VRP","POSTpaymentConsents/VRP1")
    body.data.initiation.endToEndIdentification = "01." + unixDate.getShortedDate(unixDate.getCurrentDate()) + "." + paf.generateRandomHex(16)
    body.data.initiation.instructionIdentification = "795SDBO" + unixDate.getShortedDate(unixDate.getCurrentDate()) + paf.generateRandomHex(16)
    await paf.appendToDefinedFile("logs.txt","POSTpaymentConsents/VRP1_modifiedBody",JSON.stringify(body))
    await setRequestBody("VRP","POSTpaymentConsents/VRP1",body)
}

async function generateRequestPISPauthorisationCode(config,consent_id){
    let header = {"alg":"BELTM256","typ":"JOSE"}
    let body = {"alg":"BELTM256","client_id":"PISP2TEST","consent_id":"f00cf4bb-51ef-407cbc84-c71fc6bd33ec","magic_number":"+375-255427989","max_age":0,"redirect_uri":"https://sc-map-testversion-vip.softclub.by:8008/oauth2-redirect.html","response_type":"code","scope":"SC-APPS payments openid"}
    body.client_id = "PISP2TEST"
    body.consent_id = consent_id
    let cryptoHashBody = paf.convertToBase64(paf.convertToBase64URL(JSON.stringify(header))+"."+paf.convertToBase64URL(JSON.stringify(body)))
    let dataB64 = {
        "Auth":{
            "CryptoType":3,
            "ConnectStr":"hash=1.2.112.0.2.0.34.101.31.53"
        },
        "DataB64" : cryptoHashBody,
        "MACKeyB64" : paf.convertToBase64(config.client_secret_pisp)
    }
    let res = await paf.scCryptoHash(config,dataB64)

    return paf.convertToBase64URL(JSON.stringify(header))+"."+paf.convertToBase64URL(JSON.stringify(body)) + "." + paf.convertBase64ToBase64Url(res.ResultB64)
}

async function main1() {
    const config = {
        alg: "BELTM256",
        typ: "JOSE",
        url_kc: "https://sc-map-testversion-vip.softclub.by:7891/",
        url_swagger: "https://sc-map-testversion-vip.softclub.by:8008/",
        client_id_pisp: "PISP2TEST",
        client_secret_pisp: "Cgxb4O9UWS4HZwrpbf3bfefdrZTStubt",
        client_id_qpisp: "BELKARTPAY_NPC_TEST",
        client_secret_qpisp: "aES5biV0eWVkVWUHzD36it5X2yE7DSkF",
        client_id_tpe: "ENTERPRISESOFT",
        client_secret_tpe: "Nisll6ytlAAtYGqb7W1Kus539rfLAZuP",
        client_id_dbo: "digitalChannels",
        client_secret_dbo: "rvDMLEf5Njz6L5BGpst4dLP1hMrBWxEV",
        // client_id_dbo:"iBankASB",
        // client_secret_dbo:"zoVP6g4yTFsW64a6hAbSMe43mQrWxmJl",
        apikey: "dcbeebf6-1d34-4bb0-82cf-bcfe185e037f", //V087_TEST1
        // apikey: "ed999501-fe4e-4f18-845e-d69eab692941", //test-client.12
        client_otp: "asb123",
        mobile_number: "+375-255427989"
    }

    // const config = {
    //     alg: "BELTM256",
    //     typ: "JOSE",
    //     url_kc: "https://api-test.asb.by/",
    //     url_swagger: "https://api-test.asb.by/",
    //     client_id_pisp: "PISP2TEST",
    //     client_secret_pisp: "Cgxb4O9UWS4HZwrpbf3bfefdrZTStubt",
    //     client_id_qpisp: "ERIP_ID_TEST",
    //     client_secret_qpisp: "HXA2WCWmIpbYNghWkD7KFSsDPbdUz1dI",
    //     client_id_tpe: "ENTERPRISESOFT",
    //     client_secret_tpe: "Nisll6ytlAAtYGqb7W1Kus539rfLAZuP",
    //     client_id_dbo:"digitalChannels",
    //     client_secret_dbo:"rvDMLEf5Njz6L5BGpst4dLP1hMrBWxEV",
    //     apikey: "c0bf747d-3cb0-41af-ae87-08d062517c9b", //V087_TEST1
    //     // apikey: "ed999501-fe4e-4f18-845e-d69eab692941", //test.client-12
    //     client_otp: "asb123",
    //     mobile_number: "+375-255427989"
    // }

    // const config = {
    //     alg: "BELTM256",
    //     typ: "JOSE",
    //     url_kc: "https://open-banking-akbb.softclub.by/",
    //     url_swagger: "https://open-banking-akbb.softclub.by/",
    //     // url_swagger:"192.168.166.122:10081/",
    //     client_id_pisp: "PISPTEST",
    //     client_secret_pisp: "P9XzSwdBkRw17nu1V2CSM3eogszWC0XQ",
    //     client_id_qpisp: "BELKARTPAY_NPC_TEST",
    //     client_secret_qpisp: "veZxbezDPxbOw4l0RIzfCO7TDNALvHUV",
    //     client_id_dbo: "digitalChannelsNew",
    //     client_secret_dbo: "mnoSdqcDruzrrBqev06ZLkHOGE3xRayy",
    //     client_id_tpe: "ENTERPRISESOFT",
    //     client_secret_tpe: "VUpshCxrbwRlCbTFDFpQhOUh78zeUPlU",
    //     // apikey: "0788319d-048e-449b-b785-b0e740ddc5ac", //V093_TEST1
    //     apikey: "Q3ET50eu0IgFQojiBn1M5Ypw5eRuuS90O5fhYSXMWcDFTWdtVt", //V087_TEST1
    //     client_otp: "asb123",
    //     mobile_number: "+375-255427989"
    // }
    let pispAccessToken
    let pispToken = await paf.createTokenPISP(config)
    pispAccessToken = pispToken.access_token
    await paf.appendToDefinedFile("logs.txt","PISPtoken",pispAccessToken.toString())
    console.log(pispToken)
    console.log("pisp_access_token : "+pispAccessToken)
    let dboClientAccessToken
    let dboClientToken = await paf.createDboClientToken(config)
    dboClientAccessToken = dboClientToken.access_token

    authType = "PAUapikey" // PAUapikey, OBclientCredentials
    paf.setAuthType("PAUapikey")
    // await modifyPostpaymentConsentsVRPBody()
    let createpaymentConsentsVRPResponse = await createpaymentConsentsVRP(config,pispAccessToken)
    // let createpaymentConsentsVRPResponse = {"data":{"creationDateTime":"2025-11-05T16:56:47.263849771+03:00","VRPConsentId":"447d4b11-6097-4f548073-1c21960c9661","initiation":{"amount":"100.00","creditor":{"contactDetails":{"emailAddress":"gu.gomelobl@minfin.gov.by","faxNumber":"+375-17233456780","mobileNumber":"+375-29323456789","name":"Будникова Ольга Николаевна","phoneNumber":"+375-17233456789"},"countryOfResidence":"BY","name":"Главное управление Министерства финансов РБ по Гомельской области","organisationIdentification":[{"code":"TXID","identification":"INN400555165"}],"postalAddress":{"addressLine":["246050, г. Гомель, ул. Гагарина, 17"],"buildingNumber":"17","country":"BY","countrySubDivision":"Гомель","districtName":"Гомельская область","postCode":"246050","streetName":"ул. Гагарина","townLocationName":"5000000000","townName":"г. Гомель"},"privateIdentification":[]},"creditorAccount":{"identification":"BY09AKBB36004250402980000000","schemeName":"BY.NBRB.IBAN"},"creditorAgent":{"identification":"AKBBBY2X","name":"ОАО 'АСБ Беларусбанк'"},"currency":"BYN","debtor":{"contactDetails":{"emailAddress":"aaa@mail.ru","faxNumber":"+375-18-1111111","mobileNumber":"+375-44-1111111","name":"Контактное имя","phoneNumber":"+375-17-1111111"},"countryOfResidence":"BY","name":"Митрофан Доромидонтович Белуга","organisationIdentification":[],"postalAddress":{"addressLine":["addressLine"],"buildingNumber":"1","country":"BY","countrySubDivision":"Минская область","districtName":"Минский район","postCode":"200018","room":"2","streetName":"Якубовского","townLocationName":"Фрунзенский","townName":"Минск"},"privateIdentification":[{"code":"NIDN","identification":"3010190K002PB2"}]},"debtorAccount":{"identification":"BY21AKBB30140012426250070000","schemeName":"BY.NBRB.IBAN"},"endToEndIdentification":"01.20251105.E4E8828060FFB926","instructionIdentification":"795SDBO20251105FA827234B09C1BE6","localInstrument":"BY.NBRB.BISS.NORMAL","regulatoryReporting":[],"remittanceInformation":{"categoryPurposeCode":"TAXS","proprietaryPurpose":"190401.21","referredDocument":[],"unstructured":"1Назначение платежа в неструктурированной виде длиной 140 символов 12Назначение платежа в неструктурированной виде длиной 140 символов 23Назначение платежа в неструктурированной виде длиной 140 символов 3"},"requestedExecutionDate":"2024-10-09","taxRemittance":{"additionalInformation":"a","category":"11902","creditor":{"taxIdentification":"INN101103846"},"date":"2025-08-13","debtor":{"taxIdentification":"INP511111111"},"fromDate":"2025-07-01","rate":"20.0","referenceNumber":"123","taxableBaseAmount":"1000.00","toDate":"2025-07-31","totalAmount":"200.00","type":"MM07","ultimateDebtor":{"name":"a","taxIdentification":"INP511111111"},"year":"2025-01-01"},"ultimateCreditor":{"countryOfResidence":"BY","name":"Любанский сельский исполком","organisationIdentification":[{"code":"TXID","identification":"INN400031143"}],"privateIdentification":[]}},"link":"https://dc.asb.by/paymentСonsents/VRP/447d4b11-6097-4f548073-1c21960c9661","status":"AwaitingAuthorisation","statusUpdateDateTime":"2025-11-05T16:56:47.263849771+03:00"},"links":{"self":"https://sc-map-testversion-vip.softclub.by:8008/oapi-channel/open-banking/v1.0/paymentConsents/VRP/447d4b11-6097-4f548073-1c21960c9661"},"meta":{"totalPages":1}}
    console.log(JSON.stringify(createpaymentConsentsVRPResponse))
    await paf.appendToDefinedFile("logs.txt","createpaymentConsentsVRPResponse",JSON.stringify(createpaymentConsentsVRPResponse))

    // let requestObject = await generateRequestPISPauthorisationCode(config,createpaymentConsentsVRPResponse.data.VRPConsentId)
    // console.log(JSON.stringify(requestObject))
    // await paf.appendToDefinedFile("logs.txt","requestObject_response",JSON.stringify(requestObject))

    // let accListPayments = await paf.getAccListPaymentsKEYCLOAK(config,"","V087_TEST1",config.client_id_pisp,createpaymentConsentsVRPResponse.data.VRPConsentId)
    // console.log(JSON.stringify(accListPayments))
    // await paf.appendToDefinedFile("logs.txt","accListPayments",JSON.stringify(accListPayments))

    // let accListcheck = await paf.getAccListPaymentsKEYCLOAKcheck(config,"","V087_TEST1",config.client_id_pisp,accListPayments)
    // console.log(JSON.stringify(accListcheck))
    // await paf.appendToDefinedFile("logs.txt","accListPaymentscheck",config.client_id_pisp,JSON.stringify(accListcheck))
    // let getpaymentConsentsVRPResponse = await getpaymentConsentsVRP(config,pispAccessToken,createpaymentConsentsVRPResponse.data.VRPConsentId)
    // console.log(JSON.stringify(getpaymentConsentsVRPResponse))
    // await paf.appendToDefinedFile("logs.txt","getpaymentConsentsVRPResponse",JSON.stringify(getpaymentConsentsVRPResponse))

    // let deletepaymentConsentsVRPDBOResponse = await deletepaymentConsentsVRPDBO(config,dboClientAccessToken,createpaymentConsentsVRPResponse.data.VRPConsentId)

    let getpaymentConsentsVRPDBOResponse = await getpaymentConsentsVRPDBO(config,dboClientAccessToken,createpaymentConsentsVRPResponse.data.VRPConsentId)
    console.log(JSON.stringify(getpaymentConsentsVRPDBOResponse))
    await paf.appendToDefinedFile("logs.txt","getpaymentConsentsVRPDBOResponse",JSON.stringify(getpaymentConsentsVRPDBOResponse))

    // let deletepaymentConsentsVRPResponse = await deletepaymentConsentsVRP(config,pispAccessToken,createpaymentConsentsVRPResponse.data.VRPConsentId)
    // let deletepaymentConsentsVRPDBOResponse = await deletepaymentConsentsVRPDBO(config,dboClientAccessToken,createpaymentConsentsVRPResponse.data.VRPConsentId)


    let getpaymentConsentsVRPResponse = await getpaymentConsentsVRP(config,pispAccessToken,createpaymentConsentsVRPResponse.data.VRPConsentId)
    console.log(JSON.stringify(getpaymentConsentsVRPResponse))
    await paf.appendToDefinedFile("logs.txt","getpaymentConsentsVRPResponse",JSON.stringify(getpaymentConsentsVRPResponse))

    let accListPaymentsVRP = await paf.getAccListPayments(config,dboClientAccessToken,createpaymentConsentsVRPResponse.data.VRPConsentId)
    console.log(JSON.stringify(accListPaymentsVRP))
    await paf.appendToDefinedFile("logs.txt","accListPaymentsVRP",JSON.stringify(accListPaymentsVRP))

    let patchPaymentConsentVRPResponse = await patchPaymentConsentVRP(config,dboClientAccessToken,accListPaymentsVRP)
    const endTime = performance.now()
    console.log(`Call to doSomething took ${endTime - startTime} milliseconds`)
    console.log(JSON.stringify(patchPaymentConsentVRPResponse))
    await paf.appendToDefinedFile("logs.txt","patchPaymentConsentVRPResponse",JSON.stringify(patchPaymentConsentVRPResponse))
    let deletepaymentConsentsVRPDBOResponse = await deletepaymentConsentsVRPDBO(config,dboClientAccessToken,createpaymentConsentsVRPResponse.data.VRPConsentId)
    // await modifyPostPaymentsVRPBody(createpaymentConsentsVRPResponse.data.VRPConsentId,createpaymentConsentsVRPResponse.data.initiation,createpaymentConsentsVRPResponse.risk)
    // let postPaymentsVRPResponse = await postPaymentsVRP(config,pispAccessToken)
    // console.log(JSON.stringify(postPaymentsVRPResponse))
    // await paf.appendToDefinedFile("logs.txt","createPaymentsVRPResponse",JSON.stringify(postPaymentsVRPResponse))

    getpaymentConsentsVRPDBOResponse = await getpaymentConsentsVRPDBO(config,dboClientAccessToken,createpaymentConsentsVRPResponse.data.VRPConsentId)
    console.log(JSON.stringify(getpaymentConsentsVRPDBOResponse))
    await paf.appendToDefinedFile("logs.txt","getpaymentConsentsVRPDBOResponse",JSON.stringify(getpaymentConsentsVRPDBOResponse))

    // let VRPId = postPaymentsVRPResponse.data.VRPId
    // let VRPId = "795ABSB2025110300000000002746863333"
    // let getPaymentsVRPDBOResponse = await getPaymentsVRPDBO(config,dboClientAccessToken,postPaymentsVRPResponse.data.VRPId)
    // let getPaymentsVRPDBOResponse = await getPaymentsVRPDBO(config,dboClientAccessToken,VRPId)
    // console.log(JSON.stringify(getPaymentsVRPDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentVRPDBOResponse",JSON.stringify(getPaymentsVRPDBOResponse))

    // let VRPId = "795ABSB2025110300000000002746760000"
    // let getPaymentsVRPResponse = await getPaymentsVRP(config,dboClientAccessToken,postPaymentsVRPResponse.data.VRPId)
    // console.log(JSON.stringify(getPaymentsVRPResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentVRPResponse",JSON.stringify(getPaymentsVRPResponse))

    // await deletePaymentsVRPDBO(config,dboClientAccessToken,VRPId)

    // let getPaymentsVRPDBOResponse = await getPaymentsVRPDBO(config,dboClientAccessToken,createpaymentConsentsVRPResponse.data.VRPConsentId)
    // console.log(JSON.stringify(getPaymentsVRPDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentVRPDBOResponse",JSON.stringify(getPaymentsVRPDBOResponse))

    // await deletePaymentsVRP(config,dboClientAccessToken,VRPId)
    // console.log(JSON.stringify(deletePaymentsVRPResponse))
    // await paf.appendToDefinedFile("logs.txt","deletePaymentVRPResponse",JSON.stringify(deletePaymentsVRPResponse))

    // let getPaymentsVRPDBOResponse = await getPaymentsVRPDBO(config,dboClientAccessToken,VRPId)
    // console.log(JSON.stringify(getPaymentsVRPDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentVRPDBOResponse",JSON.stringify(getPaymentsVRPDBOResponse))
    // console.log(`Call to doSomething took ${endTime - startTime} milliseconds`)
}

main1()