...
async function generateSignaturePOSTpaymentConsentsRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,additionalInfo = []){
  ...
  "typ": "JOSE"}
    ...
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,undefined,"requirement")
...
async function generateSignaturePOSTpaymentsRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,additionalInfo = []){
...
async function generateSignaturePATCHpaymentConsentsRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignatureGETpaymentConsentsRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
    ...
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementConsentId,"requirement")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
...
async function generateSignatureGETpaymentConsentsRequirementDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignatureGETpaymentsRequirementDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignatureDELETEpaymentsRequirementDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignatureGETpaymentsRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignatureDELETEpaymentsRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignatureDELETEpaymentConsentsRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignatureDELETEpaymentConsentsRequirementDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignaturePUTcreateExternalRepresentationRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignaturePUTcreateExternalRepresentationSpecialPartRequirement(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function createPaymentConsentsRequirement(config,access_token){
    let requestBodyName = "POSTpaymentConsents/requirement2"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("requirement",requestBodyName))
    ...
    let method = "POSTpaymentConsentsRequirement"
    let signature = await generateSignaturePOSTpaymentConsentsRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName)
    ...
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/requirement", {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "POST",
    mode: "cors",
    headers: headersList,
    body: JSON.stringify(paf.sortObjectAlphabeticallyNew(paf.getRequestBody("requirement",requestBodyName)))
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: POST \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("requirement",requestBodyName)))
  return response.json();
}

async function postPaymentsRequirement(config,access_token){
    let requestBodyName = "POSTpaymentsRequirement1"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("requirement",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "POSTpaymentsRequirement"
    let signature = await generateSignaturePOSTpaymentsRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
    ...
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/requirement", {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "POST",
    mode: "cors",
    headers: headersList,
...
async function generateSpecPartRequirement(config,requestBodyWithExternalRepresentation){
    let imitIns = await paf.createImitationInsert(config,requestBodyWithExternalRepresentation)
    let specPart = await createSpecialPartObjectRequirement(config,imitIns.ResultB64)
    console.log("specialPart \n" + JSON.stringify(specPart))
    setRequestBody("requirement","PUTpaymentConsentsCreateExternalRepresentationSpecialPartRequirement",specPart)
    let externalRepresentationSpecialPart = await createExternalRepresentationSpecialPartRequirement(config,specPart)
...
async function createSpecialPartObjectRequirement(config,imitIns){
...
async function createExternalRepresentationRequirement(config, requestBody){
    await paf.appendToDefinedFile("logs.txt","EXTERNAL REPRESENTATION","")
    let token = await paf.createDboClientToken(config)
    let access_token = token.access_token

    let requestBodyName = "PUTpaymentConsentsCreateExternalRepresentationRequirement"
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody = {
        "Auth":{
            "CryptoType":1,
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "PUTpaymentConsentsCreateExternalRepresentationRequirement"
    let signature = await generateSignaturePUTcreateExternalRepresentationRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,["NoIdempotencyKey"])
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
  ...
  // const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/requirement/createExternalRepresentation", {
  //   method: "PUT",
  //   mode: "cors",
  //   headers: headersList,
...
async function createExternalRepresentationSpecialPartRequirement(config, requestBody){
    await paf.appendToDefinedFile("logs.txt","EXTERNAL REPRESENTATION SPECIAL PART","")
    let token = await paf.createDboClientToken(config)
    let access_token = token.access_token

    let requestBodyName = "PUTpaymentConsentsCreateExternalRepresentationSpecialPartRequirement"
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody = {
        "Auth":{
            "CryptoType":1,
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "PUTpaymentConsentsExternalRepresentationSpecialPartRequirement"
    let signature = await generateSignaturePUTcreateExternalRepresentationSpecialPartRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,["NoIdempotencyKey"])
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
...
async function generatePaymentConsentRequirementAuthoriseBody(config,accList,createPaymentConsentsRequirementResponse){
    ...
    setRequestBody("requirement","PUTpaymentConsentsCreateExternalRepresentationRequirement",requestBody)
    let latestAuthType = authType;
    authType = "OBclientCredentials"
    let externalRepresentation = await createExternalRepresentationRequirement(config, requestBody)
    ...
    authType = "OBclientCredentials"
    let specPart = await generateSpecPartRequirement(config, requestBody)
    authType = latestAuthType
    requestBody.specialPart = specPart.specialPart
    requestBody = paf.renameKeyInObject(requestBody,"paymentConsentId","requirementConsentId")
    requestBody = paf.sortObjectAlphabeticallyNew(requestBody)

    return requestBody
}

async function patchPaymentConsentRequirement(config,access_token,accList,createPaymentConsentsRequirementResponse){
    let requestBodyName = "PATCHpaymentConsents/requirement1"
    let requestBody = await generatePaymentConsentRequirementAuthoriseBody(config,accList,createPaymentConsentsRequirementResponse)
    setRequestBody("requirement",requestBodyName,requestBody)
    let savedBody = paf.getRequestBody("requirement",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "PATCHpaymentConsentsRequirement"
    let signature = await generateSignaturePATCHpaymentConsentsRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
...
async function getPaymentConsentsRequirement(config,access_token,RequirementConsentId){
    let requestBodyName = "GETpaymentConsents/requirement"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("requirement",requestBodyName))
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentConsentsRequirement"
    let signature = await generateSignatureGETpaymentConsentsRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
    ...
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/requirement/" + RequirementConsentId, {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "GET",
    mode: "cors",
    headers: headersList,
...
async function getPaymentConsentsRequirementDBO(config,access_token,RequirementConsentId){
    let latestAuthType = authType
    let requestBodyName = "GETpaymentConsents/requirementDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("requirement",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    authType = "OBclientCredentials"
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentConsentsRequirementDBO"
    let signature = await generateSignatureGETpaymentConsentsRequirementDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
    ...
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/requirement/" + RequirementConsentId +"/PSUorPAU/V087_TEST1", {
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
...
async function getPaymentsRequirementDBO(config,access_token,RequirementId){
    let latestAuthType = authType
    let requestBodyName = "GETpayments/requirementDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("requirement",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    authType = "OBclientCredentials"
    paf.setAuthType("OBclientCredentials")
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentsRequirementDBO"
    let signature = await generateSignatureGETpaymentsRequirementDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
    ...
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/requirement/" + RequirementId +"/PSUorPAU/V087_TEST1", {
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
...
async function deletePaymentsRequirementDBO(config,access_token,RequirementId){
    let latestAuthType = authType
    let requestBodyName = "DELETEpayments/RequirementDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("requirement",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    authType = "OBclientCredentials"
    paf.setAuthType("OBclientCredentials")
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentsRequirementDBO"
    let signature = await generateSignatureDELETEpaymentsRequirementDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
...
async function getPaymentsRequirement(config,access_token,RequirementId){
    let requestBodyName = "GETpayments/requirement"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("requirement",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentsRequirement"
    let signature = await generateSignatureGETpaymentsRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
    ...
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/requirement/" + RequirementId, {
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
...
async function deletePaymentsRequirement(config,access_token,RequirementId){
    let requestBodyName = "DELETEpayments/Requirement"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("requirement",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentsRequirement"
    let signature = await generateSignatureDELETEpaymentsRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
...
async function deletePaymentConsentsRequirement(config,access_token,RequirementConsentId){
    let requestBodyName = "DELETEpaymentConsentsRequirement"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("requirement",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentConsentsRequirement"
    let signature = await generateSignatureDELETEpaymentConsentsRequirement(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
      let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
...
async function deletePaymentConsentsRequirementDBO(config,access_token,RequirementConsentId){
    let prevAuthType = authType
    authType = "OBclientCredentials"
    setAuthType("OBclientCredentials")
    let requestBodyName = "DELETEpaymentConsentsRequirementDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("requirement",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentConsentsRequirementDBO"
    let signature = await generateSignatureDELETEpaymentConsentsRequirementDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,RequirementConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
      let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
...
async function modifyPATCHpaymentConsentRequirementBody(endToEndIdentification,instructionIdentification){
...
async function modifyPostPaymentsRequirementBody(requirementConsentId,initiation,risk){
    let body = await paf.getRequestBody("requirement","POSTpaymentsRequirement1")
    body.data.requirementConsentId = requirementConsentId
    body.data.initiation = initiation
    body.risk = risk
    await paf.appendToDefinedFile("logs.txt","POSTpaymentsRequirement1_modifiedBody",JSON.stringify(body))
    await setRequestBody("requirement","POSTpaymentsRequirement1",body)
}
...
async function main1() {
    ...
    let createPaymentConsentsRequirementResponse = await createPaymentConsentsRequirement(config,pispAccessToken)
    // let createPaymentConsentsRequirementResponse = {"data":{"charge":[],"creationDateTime":"2025-10-20T19:19:23.514437627+03:00","initiation":{"amount":"201.00","creditor":{"countryOfResidence":"BY","name":"ST-”KRYSHKOVSKAYA short“ короткое наименование","organisationIdentification":[{"code":"TXID","identification":"INN101103846"}],"privateIdentification":[]},"creditorAccount":{"identification":"BY48AKBB30121272200458000933","schemeName":"BY.NBRB.IBAN"},"creditorAgent":{"identification":"AKBBBY2X","name":"ОАО 'АСБ Беларусбанк'"},"currency":"BYN","endToEndIdentification":"01.20250412.8bd17819bcf8d8da","instructionIdentification":"795SDBO20250412E5AE1D70F25F431F84DD","requirement":[{"IBAN":"BY21AKBB30140012426250070000","amount":"100.00","name":"Белуга Митрофан Доромидонтович"},{"IBAN":"BY06AKBB30140012426260070000","amount":"101.00","name":"Килька Авдотья Никитишна"}],"localInstrument":"BY.NBRB.BISS.NORMAL","requestedExecutionDate":"2025-12-12"},"link":"https://dc.asb.by/paymentСonsents/requirement/15582e04-8249-4957a675-0ba7c95535fb","RequirementConsentId":"15582e04-8249-4957a675-0ba7c95535fb","status":"AwaitingAuthorisation","statusUpdateDateTime":"2025-10-20T19:19:23.514437627+03:00"},"links":{"self":"https://sc-map-testversion-vip.softclub.by:8008/oapi-channel/open-banking/v1.0/paymentConsents/requirement/15582e04-8249-4957a675-0ba7c95535fb"},"meta":{"totalPages":1}}
    console.log(JSON.stringify(createPaymentConsentsRequirementResponse))
    await paf.appendToDefinedFile("logs.txt","createPaymentConsentsRequirementResponse",JSON.stringify(createPaymentConsentsRequirementResponse))

    // let getPaymentConsentsRequirementResponse = await getPaymentConsentsRequirement(config,pispAccessToken,createPaymentConsentsRequirementResponse.data.requirementConsentId)
    // console.log(JSON.stringify(getPaymentConsentsRequirementResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentConsentsRequirementResponse",JSON.stringify(getPaymentConsentsRequirementResponse))

    // let getPaymentConsentsRequirementDBOResponse = await getPaymentConsentsRequirementDBO(config,dboClientAccessToken,createPaymentConsentsRequirementResponse.data.requirementConsentId)
    // console.log(JSON.stringify(getPaymentConsentsRequirementDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentConsentsRequirementDBOResponse",JSON.stringify(getPaymentConsentsRequirementDBOResponse))

    // let deletePaymentConsentsRequirementResponse = await deletePaymentConsentsRequirement(config,pispAccessToken,createPaymentConsentsRequirementResponse.data.requirementConsentId)
    // let deletePaymentConsentsRequirementDBOResponse = await deletePaymentConsentsRequirementDBO(config,dboClientAccessToken,createPaymentConsentsRequirementResponse.data.requirementConsentId)

    // getPaymentConsentsRequirementResponse = await getPaymentConsentsRequirement(config,pispAccessToken,createPaymentConsentsRequirementResponse.data.requirementConsentId)
    // console.log(JSON.stringify(getPaymentConsentsRequirementResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentConsentsRequirementResponse",JSON.stringify(getPaymentConsentsRequirementResponse))

    let accListPaymentsRequirement = await paf.getAccListPayments(config,dboClientAccessToken,createPaymentConsentsRequirementResponse.data.requirementConsentId)
    console.log(JSON.stringify(accListPaymentsRequirement))
    await paf.appendToDefinedFile("logs.txt","accListPaymentsRequirement",JSON.stringify(accListPaymentsRequirement))


    let patchPaymentConsentRequirementResponse = await patchPaymentConsentRequirement(config,dboClientAccessToken,accListPaymentsRequirement,createPaymentConsentsRequirementResponse)
    console.log(JSON.stringify(patchPaymentConsentRequirementResponse))
    await paf.appendToDefinedFile("logs.txt","patchPaymentConsentRequirementResponse",JSON.stringify(patchPaymentConsentRequirementResponse))

    // await modifyPostPaymentsRequirementBody(createPaymentConsentsRequirementResponse.data.requirementConsentId,createPaymentConsentsRequirementResponse.data.initiation,createPaymentConsentsRequirementResponse.risk)
    // let postPaymentsRequirementResponse = await postPaymentsRequirement(config,pispAccessToken)
    // console.log(JSON.stringify(postPaymentsRequirementResponse))
    // await paf.appendToDefinedFile("logs.txt","createPaymentsRequirementResponse",JSON.stringify(postPaymentsRequirementResponse))

    // let requirementId = postPaymentsRequirementResponse.data.requirementId
    // let getPaymentsRequirementDBOResponse = await getPaymentsRequirementDBO(config,dboClientAccessToken,requirementId)
    // console.log(JSON.stringify(getPaymentsRequirementDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentRequirementDBOResponse",JSON.stringify(getPaymentsRequirementDBOResponse))

    // let getPaymentsRequirementResponse = await getPaymentsRequirement(config,dboClientAccessToken,requirementId)
    // console.log(JSON.stringify(getPaymentsRequirementResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentRequirementResponse",JSON.stringify(getPaymentsRequirementResponse))

    // await deletePaymentsRequirementDBO(config,dboClientAccessToken,RequirementId)
    // console.log(JSON.stringify(deletePaymentsRequirementDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","deletePaymentRequirementDBOResponse",JSON.stringify(deletePaymentsRequirementDBOResponse))

    // getPaymentsRequirementDBOResponse = await getPaymentsRequirementDBO(config,dboClientAccessToken,requirementId)
    // console.log(JSON.stringify(getPaymentsRequirementDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentRequirementDBOResponse",JSON.stringify(getPaymentsRequirementDBOResponse))

    // await deletePaymentsRequirement(config,dboClientAccessToken,RequirementId)
    // console.log(JSON.stringify(deletePaymentsRequirementResponse))
    // await paf.appendToDefinedFile("logs.txt","deletePaymentRequirementResponse",JSON.stringify(deletePaymentsRequirementResponse))
