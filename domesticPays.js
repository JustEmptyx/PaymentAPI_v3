const uuid = require("uuid");
const paf = require("./PISPauth")
const {setRequestBody} = require("./requestBodies");
const {format} = require("date-fns");
const {setAuthType} = require("./PISPauth");
let unixDate = new paf.UnixDate()

let startTime = ""
let authType = "PAUapikey" // PAUapikey, OBclientCredentials
paf.setAuthType("PAUapikey")
process.env["NODE_TLS_REJECT_UNAUTHORIZED"] = 0;

async function generateSignaturePOSTpaymentConsentsDomesticTax(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "content-type",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "content-type",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,undefined,"pispAuthTax")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignaturePOSTpaymentsDomesticTax(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "content-type",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "content-type",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,undefined,"pispAuthTax")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignaturePATCHpaymentConsentsDomesticTax(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "content-type",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "content-type",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
      "x-idempotency-key"
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,undefined,"pispAuthTax")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureGETpaymentConsentsDomesticTax(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,"pispAuthTax")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureGETpaymentConsentsDomesticTaxDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,"pispAuthTax")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureGETpaymentsDomesticTaxDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,"pispAuthTax")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureDELETEpaymentsDomesticTaxDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,"pispAuthTax")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureGETpaymentsDomesticTax(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,"pispAuthTax")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureDELETEpaymentsDomesticTax(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,"pispAuthTax")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}


async function generateSignatureDELETEpaymentConsentsDomesticTax(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,"pispAuthTax")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignatureDELETEpaymentConsentsDomesticTaxDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,"pispAuthTax")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignaturePUTcreateExternalRepresentationDomesticTax(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "content-type",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "content-type",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,"pispAuthTax")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}

async function generateSignaturePUTcreateExternalRepresentationSpecialPartDomesticTax(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "authorization",
      "content-digest",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    } else {
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
      "@target-uri",
      "content-digest",
      "x-api-key",
      "x-fapi-auth-date",
      "x-fapi-customer-ip-address",
      "x-fapi-interaction-id",
    ]
    }
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayload(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId,"pispAuthTax")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
    await paf.appendToDefinedFile("logs.txt","headerB64",JSON.stringify(headerB64))
    await paf.appendToDefinedFile("logs.txt","payloadB64",JSON.stringify(payloadB64))
    let signBody = {
        "Auth":{
            "CryptoType":1,
            "KeyID":"B6D7498EE0E67A368E921810E53849B0C7A69646",
            "Password":"12345678"
        },
        "DataB64": paf.convertToBase64(headerB64 + "." + payloadB64),
        "OptAddAllCert":false,
        "OptAddCert":true,
        "OptCheckPrivateKey":true,
        "OptReturnSignCert":true
    }
    await paf.appendToDefinedFile("logs.txt","signBody",JSON.stringify(signBody))
    console.log("DataB64 \r\n" + paf.convertToBase64(headerB64 + "." + payloadB64))
    let hash = await paf.scCryptoSign(config,signBody)
    await paf.appendToDefinedFile("logs.txt","signedBody",JSON.stringify(hash))
    console.log("signedData" + JSON.stringify(hash))
    let signature = headerB64 +".."+ paf.convertBase64ToBase64Url(hash.ResultB64)

    return signature
}


async function createPaymentConsentsDomesticTax(config,access_token){
    let requestBodyName = "POSTpaymentConsents/domesticTax2"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("pispAuthTax",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "POSTpaymentConsentsDomesticTax"
    let signature = await generateSignaturePOSTpaymentConsentsDomesticTax(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/domesticTax", {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "POST",
    mode: "cors",
    headers: headersList,
    body: JSON.stringify(paf.sortObjectAlphabeticallyNew(paf.getRequestBody("pispAuthTax",requestBodyName)))
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: POST \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("pispAuthTax",requestBodyName)))
  return response.json();
}

async function postPaymentsDomesticTax(config,access_token){
    let requestBodyName = "POSTpaymentsDomesticTax1"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("pispAuthTax",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "POSTpaymentsDomesticTax"
    let signature = await generateSignaturePOSTpaymentsDomesticTax(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/domesticTax", {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "POST",
    mode: "cors",
    headers: headersList,
    body: JSON.stringify(paf.sortObjectAlphabeticallyNew(paf.getRequestBody("pispAuthTax",requestBodyName)))
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: POST \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("pispAuthTax",requestBodyName)))
  return response.json();
}

async function generateSpecPartDomesticTax(config,requestBodyWithExternalRepresentation){
    let imitIns = await paf.createImitationInsert(config,requestBodyWithExternalRepresentation)
    let specPart = await createSpecialPartObjectDomesticTax(config,imitIns.ResultB64)
    console.log("specialPart \n" + JSON.stringify(specPart))
    setRequestBody("pispAuthTax","PUTpaymentConsentsCreateExternalRepresentationSpecialPartDomesticTax",specPart)
    let externalRepresentationSpecialPart = await createExternalRepresentationSpecialPartDomesticTax(config,specPart)
    specPart.specialPart.externalRepresentationSpecialPart = externalRepresentationSpecialPart.data.externalRepresentationSpecialPart
    return specPart
}

async function createSpecialPartObjectDomesticTax(config,imitIns){
  const currentDate = new Date();
  const formattedDate = format(currentDate, "yyyy-MM-dd'T'HH:mm:ssXXX");
  currentDate.setMinutes(currentDate.getMinutes()+2)
  const updatedFormattedDate = format(currentDate, "yyyy-MM-dd'T'HH:mm:ssXXX");
  return {
    "specialPart":
        {
          "HMAC": imitIns,
          "OTP": config.client_otp,
          "OTPdateTime": formattedDate,
          "mobileNumber": config.mobile_number,
          "signatureDateTime": updatedFormattedDate,
          "status": "Authorised",
          "statusUpdateDateTime": formattedDate,
          "subjectKeyIdentifier": "B6D7498EE0E67A368E921810E53849B0C7A69646"
        }
  }
}
async function createExternalRepresentationDomesticTax(config, requestBody){
    await paf.appendToDefinedFile("logs.txt","EXTERNAL REPRESENTATION","")
    let token = await paf.createDboClientToken(config)
    let access_token = token.access_token

    let requestBodyName = "PUTpaymentConsentsCreateExternalRepresentationDomesticTax"
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody = {
        "Auth":{
            "CryptoType":1,
        },
        "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64

    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "PUTpaymentConsentsCreateExternalRepresentationDomesticTax"
    let signature = await generateSignaturePUTcreateExternalRepresentationDomesticTax(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,["NoIdempotencyKey"])
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))

    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":"
    }
    console.log(JSON.stringify(headersList))
    await paf.appendToDefinedFile("logs.txt","fullRequest","method: PUT \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("pispAuthTax",requestBodyName)))
  const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/domesticTax/createExternalRepresentation", {
    method: "PUT",
    mode: "cors",
    headers: headersList,
    body: JSON.stringify(requestBody)
  });
  // let response = {"data":{"externalRepresentation":""}}
  return response.json();
  // return response
}

async function createExternalRepresentationSpecialPartDomesticTax(config, requestBody){
    await paf.appendToDefinedFile("logs.txt","EXTERNAL REPRESENTATION SPECIAL PART","")
    let token = await paf.createDboClientToken(config)
    let access_token = token.access_token

    let requestBodyName = "PUTpaymentConsentsCreateExternalRepresentationSpecialPartDomesticTax"
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody = {
        "Auth":{
            "CryptoType":1,
        },
        "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64

    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "PUTpaymentConsentsExternalRepresentationSpecialPartDomesticTax"
    let signature = await generateSignaturePUTcreateExternalRepresentationSpecialPartDomesticTax(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,["NoIdempotencyKey"])
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))

    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    console.log(JSON.stringify(headersList))
  //   const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/createSpecialPartExternalRepresentation", {
  //   method: "PUT",
  //   mode: "cors",
  //   headers: headersList,
  //   body:JSON.stringify(requestBody)
  // });
    let response = {"data":{"externalRepresentationSpecialPart":""}}
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: POST \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("pispAuthTax",requestBodyName)))
  // return response.json();
    return response
}

async function generatePaymentConsentDomesticTaxAuthoriseBody(config,accList){
    delete accList.data.account
    delete accList.data.initiation.enclosedFile
    delete accList.data.initiation.listAccounts
    delete accList.data.initiation.listPassportData
    delete accList.data.charge
    accList.data
    // accList.data.statusUpdateDateTime = accList.data.statusUpdateDateTime.split('+')[0]
    // accList.data.creationDateTime = accList.data.creationDateTime.split('+')[0]
    let sortedAccList = paf.sortObjectAlphabeticallyNew(accList)
    let requestBody = sortedAccList
    requestBody = paf.renameKeyInObject(requestBody,"paymentConsentId","domesticTaxConsentId")
    requestBody = paf.sortObjectAlphabeticallyNew(requestBody)
    setRequestBody("pispAuthTax","PUTpaymentConsentsCreateExternalRepresentationDomesticTax",requestBody)
    paf.setAuthType("OBclientCredentials")
    let externalRepresentation = await createExternalRepresentationDomesticTax(config, requestBody)
    requestBody.data.externalRepresentation = externalRepresentation.data.externalRepresentation
    requestBody = paf.sortObjectAlphabeticallyNew(requestBody)

    let specPart = await generateSpecPartDomesticTax(config, requestBody)
    requestBody.specialPart = specPart.specialPart
    requestBody = paf.sortObjectAlphabeticallyNew(requestBody)
    return requestBody
}

async function patchPaymentConsentDomesticTax(config,access_token,accList){
    let latestAuthType = authType
    authType = "OBclientCredentials"
    setAuthType("OBclientCredentials")
    let requestBodyName = "PATCHpaymentConsents/domesticTax1"
    let requestBody = await generatePaymentConsentDomesticTaxAuthoriseBody(config,accList)
    setRequestBody("pispAuthTax",requestBodyName,requestBody)
    let savedBody = paf.getRequestBody("pispAuthTax",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "PATCHpaymentConsentsDomesticTax"
    let signature = await generateSignaturePATCHpaymentConsentsDomesticTax(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-idempotency-key": idempotencyKey,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    startTime = performance.now()
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/domesticTax", {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "PATCH",
    mode: "cors",
    headers: headersList,
    body: JSON.stringify(paf.sortObjectAlphabeticallyNew(paf.getRequestBody("pispAuthTax",requestBodyName)))
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: PATCH \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("pispAuthTax",requestBodyName)))
  authType = latestAuthType
  setAuthType(latestAuthType)
  return response.json();
}

async function getPaymentConsentsDomesticTax(config,access_token,domesticTaxConsentId){
    let requestBodyName = "GETpaymentConsents/domesticTax"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("pispAuthTax",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentConsentsDomesticTax"
    let signature = await generateSignatureGETpaymentConsentsDomesticTax(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/domesticTax/" + domesticTaxConsentId, {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("pispAuthTax",requestBodyName)))
  return response.json();
}

async function getPaymentConsentsDomesticTaxDBO(config,access_token,domesticTaxConsentId){
    let latestAuthType = authType
    let requestBodyName = "GETpaymentConsents/domesticTaxDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("pispAuthTax",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    authType = "OBclientCredentials"
    paf.setAuthType("OBclientCredentials")
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentConsentsDomesticTaxDBO"
    let signature = await generateSignatureGETpaymentConsentsDomesticTaxDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/domesticTax/" + domesticTaxConsentId +"/PSUorPAU/V087_TEST1", {
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("pispAuthTax",requestBodyName)))
  authType = latestAuthType
  paf.setAuthType(latestAuthType)
  return response.json();
}

async function getPaymentsDomesticTaxDBO(config,access_token,domesticTaxId){
    let latestAuthType = authType
    let requestBodyName = "GETpayments/domesticTaxDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("pispAuthTax",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    authType = "OBclientCredentials"
    paf.setAuthType("OBclientCredentials")
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentsDomesticTaxDBO"
    let signature = await generateSignatureGETpaymentsDomesticTaxDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/domesticTax/" + domesticTaxId +"/PSUorPAU/V087_TEST1", {
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("pispAuthTax",requestBodyName)))
  authType = latestAuthType
  paf.setAuthType(latestAuthType)
  return response.json();
}

async function deletePaymentsDomesticTaxDBO(config,access_token,domesticTaxId){
    let latestAuthType = authType
    let requestBodyName = "DELETEpayments/domesticTaxDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("pispAuthTax",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    authType = "OBclientCredentials"
    paf.setAuthType("OBclientCredentials")
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentsDomesticTaxDBO"
    let signature = await generateSignatureDELETEpaymentsDomesticTaxDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/domesticTax/" + domesticTaxId +"/PSUorPAU/V087_TEST1", {
    method: "DELETE",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("pispAuthTax",requestBodyName)))
  authType = latestAuthType
  paf.setAuthType(latestAuthType)
  return response.json();
}

async function getPaymentsDomesticTax(config,access_token,domesticTaxId){
    let requestBodyName = "GETpayments/domesticTax"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("pispAuthTax",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentsDomesticTax"
    let signature = await generateSignatureGETpaymentsDomesticTax(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/domesticTax/" + domesticTaxId, {
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("pispAuthTax",requestBodyName)))
  return response.json();
}

async function deletePaymentsDomesticTax(config,access_token,domesticTaxId){
    let requestBodyName = "DELETEpayments/domesticTax"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("pispAuthTax",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentsDomesticTax"
    let signature = await generateSignatureDELETEpaymentsDomesticTax(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/domesticTax/" + domesticTaxId, {
    method: "DELETE",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: GET \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("pispAuthTax",requestBodyName)))
  return response.json();
}

async function deletePaymentConsentsDomesticTax(config,access_token,domesticTaxConsentId){
    let requestBodyName = "DELETEpaymentConsentsDomesticTax"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("pispAuthTax",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentConsentsDomesticTax"
    let signature = await generateSignatureDELETEpaymentConsentsDomesticTax(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
      let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/domesticTax/" + domesticTaxConsentId, {
    method: "DELETE",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: DELETE \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("pispAuthTax",requestBodyName)))
  // return response.json();
}

async function deletePaymentConsentsDomesticTaxDBO(config,access_token,domesticTaxConsentId){
    let prevAuthType = authType
    authType = "OBclientCredentials"
    setAuthType("OBclientCredentials")
    let requestBodyName = "DELETEpaymentConsentsDomesticTaxDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("pispAuthTax",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": paf.convertToBase64(JSON.stringify(requestBody))
        }
    } else {
        cryptoHashBody = {
            "Auth":{
                "CryptoType":1,
            },
            "DataB64": ""
        }
    }
    await paf.appendToDefinedFile("logs.txt","cryptoHashBody",JSON.stringify(cryptoHashBody))
    let hmac = await paf.scCryptoHash(config,cryptoHashBody)
    hmac = hmac.ResultB64
    await paf.appendToDefinedFile("logs.txt","content-digest",hmac.toString())
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentConsentsDomesticTaxDBO"
    let signature = await generateSignatureDELETEpaymentConsentsDomesticTaxDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,domesticTaxConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
      let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Authorization': 'Bearer '+ access_token,
      'Accept-Language': 'ru',
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    } : {
      "Content-Type": "application/json;charset=utf-8",
      'accept': 'application/json;charset=utf-8',
      'Accept-Language': 'ru',
      'x-api-key': config.apikey,
      "x-fapi-auth-date": unixDate.getFormattedDate(unixDate.getDateNsecondsAgo(600)),
      "x-fapi-customer-ip-address": "192.168.247.72",
      "x-fapi-interaction-id": fapiInteractionId,
      "x-jws-signature": signature,
      "content-digest": "belt-hash256=:"+ hmac +":",
    }
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    // const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/domesticTax/" + domesticTaxConsentId + "/PSUorPAU/V087_TEST1", {
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/domesticTax/" + domesticTaxConsentId + "/PSUorPAU/V087_TEST1", {
    method: "DELETE",
    mode: "cors",
    headers: headersList,
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: DELETE \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("pispAuthTax",requestBodyName)))
  authType = prevAuthType
  setAuthType(prevAuthType)
  // return response.json();
}

async function modifyPATCHpaymentConsentDomesticTaxBody(uri_paymentLink,instantConsentId,invoiceInstantId,instructionIdentification,endToEndIdentification){
    let body = await paf.getRequestBody("tpeAuth","PATCHpaymentInstant/invoice1")
    body.data.paymentLink[0]["URI"] = uri_paymentLink
    body.data.instantConsentId = instantConsentId
    body.data.invoiceInstantId = invoiceInstantId
    body.data.modification.paymentEndToEndIdentification = endToEndIdentification
    body.data.modification.paymentInstructionIdentification = instructionIdentification
    await paf.appendToDefinedFile("logs.txt","patchInstantInvoice_modifiedBody",JSON.stringify(body))
    await paf.setRequestBody("tpeAuth","PATCHpaymentInstant/invoice1",body)
}

async function modifyPostPaymentsDomesticTaxBody(domesticTaxConsentId,initiation){
    let body = await paf.getRequestBody("pispAuthTax","POSTpaymentsDomesticTax1")
    body.data.domesticTaxConsentId = domesticTaxConsentId
    body.data.initiation = initiation
    await paf.appendToDefinedFile("logs.txt","POSTpaymentsDomesticTax1_modifiedBody",JSON.stringify(body))
    await setRequestBody("pispAuthTax","POSTpaymentsDomesticTax1",body)
}

async function modifyPostPaymentConsentsDomesticTaxBody(domesticTaxConsentId,initiation){
    let body = await paf.getRequestBody("pispAuthTax","POSTpaymentConsents/domesticTax1")
    body.data.initiation.endToEndIdentification = "01." + unixDate.getShortedDate(unixDate.getCurrentDate()) + "." + paf.generateRandomHex(16)
    body.data.initiation.instructionIdentification = "795SDBO" + unixDate.getShortedDate(unixDate.getCurrentDate()) + paf.generateRandomHex(16)
    await paf.appendToDefinedFile("logs.txt","POSTpaymentConsents/domesticTax1_modifiedBody",JSON.stringify(body))
    await setRequestBody("pispAuthTax","POSTpaymentConsents/domesticTax1",body)
}

async function generateRequestPISPauthorisationCode(config,consent_id){
    let header = {"alg":"BELTM256","typ":"JOSE"}
    let body = {"alg":"BELTM256","client_id":"PISP2TEST","consent_id":"f00cf4bb-51ef-407cbc84-c71fc6bd33ec","magic_number":"+375-255427989","max_age":0,"redirect_uri":"https://sc-map-testversion-vip.softclub.by:8008/oauth2-redirect.html","response_type":"code","scope":"SC-APPS payments openid"}
    body.client_id = "PISP2TEST"
    body.consent_id = consent_id
    let cryptoHashBody = paf.convertToBase64(paf.convertToBase64URL(JSON.stringify(header))+"."+paf.convertToBase64URL(JSON.stringify(body)))
    let dataB64 = {
        "Auth":{
            "CryptoType":3,
            "ConnectStr":"hash=1.2.112.0.2.0.34.101.31.53"
        },
        "DataB64" : cryptoHashBody,
        "MACKeyB64" : paf.convertToBase64(config.client_secret_pisp)
    }
    let res = await paf.scCryptoHash(config,dataB64)

    return paf.convertToBase64URL(JSON.stringify(header))+"."+paf.convertToBase64URL(JSON.stringify(body)) + "." + paf.convertBase64ToBase64Url(res.ResultB64)
}

async function main1() {
    const config = {
        alg: "BELTM256",
        typ: "JOSE",
        url_kc: "https://sc-map-testversion-vip.softclub.by:7891/",
        url_swagger: "https://sc-map-testversion-vip.softclub.by:8008/",
        client_id_pisp: "PISP2TEST",
        client_secret_pisp: "Cgxb4O9UWS4HZwrpbf3bfefdrZTStubt",
        client_id_qpisp: "BELKARTPAY_NPC_TEST",
        client_secret_qpisp: "aES5biV0eWVkVWUHzD36it5X2yE7DSkF",
        client_id_tpe: "ENTERPRISESOFT",
        client_secret_tpe: "Nisll6ytlAAtYGqb7W1Kus539rfLAZuP",
        client_id_dbo: "digitalChannels",
        client_secret_dbo: "rvDMLEf5Njz6L5BGpst4dLP1hMrBWxEV",
        // client_id_dbo:"iBankASB",
        // client_secret_dbo:"zoVP6g4yTFsW64a6hAbSMe43mQrWxmJl",
        apikey: "dcbeebf6-1d34-4bb0-82cf-bcfe185e037f", //V087_TEST1
        // apikey: "ed999501-fe4e-4f18-845e-d69eab692941", //test-client.12
        client_otp: "asb123",
        mobile_number: "+375-255427989"
    }

    // const config = {
    //     alg: "BELTM256",
    //     typ: "JOSE",
    //     url_kc: "https://api-test.asb.by/",
    //     url_swagger: "https://api-test.asb.by/",
    //     client_id_pisp: "PISP2TEST",
    //     client_secret_pisp: "Cgxb4O9UWS4HZwrpbf3bfefdrZTStubt",
    //     client_id_qpisp: "ERIP_ID_TEST",
    //     client_secret_qpisp: "HXA2WCWmIpbYNghWkD7KFSsDPbdUz1dI",
    //     client_id_tpe: "ENTERPRISESOFT",
    //     client_secret_tpe: "Nisll6ytlAAtYGqb7W1Kus539rfLAZuP",
    //     client_id_dbo:"digitalChannels",
    //     client_secret_dbo:"rvDMLEf5Njz6L5BGpst4dLP1hMrBWxEV",
    //     apikey: "c0bf747d-3cb0-41af-ae87-08d062517c9b", //V087_TEST1
    //     // apikey: "ed999501-fe4e-4f18-845e-d69eab692941", //test.client-12
    //     client_otp: "asb123",
    //     mobile_number: "+375-255427989"
    // }

    // const config = {
    //     alg: "BELTM256",
    //     typ: "JOSE",
    //     url_kc: "https://open-banking-akbb.softclub.by/",
    //     url_swagger: "https://open-banking-akbb.softclub.by/",
    //     // url_swagger:"192.168.166.122:10081/",
    //     client_id_pisp: "PISPTEST",
    //     client_secret_pisp: "P9XzSwdBkRw17nu1V2CSM3eogszWC0XQ",
    //     client_id_qpisp: "BELKARTPAY_NPC_TEST",
    //     client_secret_qpisp: "veZxbezDPxbOw4l0RIzfCO7TDNALvHUV",
    //     client_id_dbo: "digitalChannelsNew",
    //     client_secret_dbo: "mnoSdqcDruzrrBqev06ZLkHOGE3xRayy",
    //     client_id_tpe: "ENTERPRISESOFT",
    //     client_secret_tpe: "VUpshCxrbwRlCbTFDFpQhOUh78zeUPlU",
    //     // apikey: "0788319d-048e-449b-b785-b0e740ddc5ac", //V093_TEST1
    //     apikey: "Q3ET50eu0IgFQojiBn1M5Ypw5eRuuS90O5fhYSXMWcDFTWdtVt", //V087_TEST1
    //     client_otp: "asb123",
    //     mobile_number: "+375-255427989"
    // }
    let pispAccessToken
    let pispToken = await paf.createTokenPISP(config)
    pispAccessToken = pispToken.access_token
    await paf.appendToDefinedFile("logs.txt","PISPtoken",pispAccessToken.toString())
    console.log(pispToken)
    console.log("pisp_access_token : "+pispAccessToken)
    let dboClientAccessToken
    let dboClientToken = await paf.createDboClientToken(config)
    dboClientAccessToken = dboClientToken.access_token

    authType = "PAUapikey" // PAUapikey, OBclientCredentials
    paf.setAuthType("PAUapikey")
    await modifyPostPaymentConsentsDomesticTaxBody()
    let createPaymentConsentsDomesticTaxResponse = await createPaymentConsentsDomesticTax(config,pispAccessToken)
    // let createPaymentConsentsDomesticTaxResponse = {"data":{"creationDateTime":"2025-11-05T16:56:47.263849771+03:00","domesticTaxConsentId":"447d4b11-6097-4f548073-1c21960c9661","initiation":{"amount":"100.00","creditor":{"contactDetails":{"emailAddress":"gu.gomelobl@minfin.gov.by","faxNumber":"+375-17233456780","mobileNumber":"+375-29323456789","name":"Будникова Ольга Николаевна","phoneNumber":"+375-17233456789"},"countryOfResidence":"BY","name":"Главное управление Министерства финансов РБ по Гомельской области","organisationIdentification":[{"code":"TXID","identification":"INN400555165"}],"postalAddress":{"addressLine":["246050, г. Гомель, ул. Гагарина, 17"],"buildingNumber":"17","country":"BY","countrySubDivision":"Гомель","districtName":"Гомельская область","postCode":"246050","streetName":"ул. Гагарина","townLocationName":"5000000000","townName":"г. Гомель"},"privateIdentification":[]},"creditorAccount":{"identification":"BY09AKBB36004250402980000000","schemeName":"BY.NBRB.IBAN"},"creditorAgent":{"identification":"AKBBBY2X","name":"ОАО 'АСБ Беларусбанк'"},"currency":"BYN","debtor":{"contactDetails":{"emailAddress":"aaa@mail.ru","faxNumber":"+375-18-1111111","mobileNumber":"+375-44-1111111","name":"Контактное имя","phoneNumber":"+375-17-1111111"},"countryOfResidence":"BY","name":"Митрофан Доромидонтович Белуга","organisationIdentification":[],"postalAddress":{"addressLine":["addressLine"],"buildingNumber":"1","country":"BY","countrySubDivision":"Минская область","districtName":"Минский район","postCode":"200018","room":"2","streetName":"Якубовского","townLocationName":"Фрунзенский","townName":"Минск"},"privateIdentification":[{"code":"NIDN","identification":"3010190K002PB2"}]},"debtorAccount":{"identification":"BY21AKBB30140012426250070000","schemeName":"BY.NBRB.IBAN"},"endToEndIdentification":"01.20251105.E4E8828060FFB926","instructionIdentification":"795SDBO20251105FA827234B09C1BE6","localInstrument":"BY.NBRB.BISS.NORMAL","regulatoryReporting":[],"remittanceInformation":{"categoryPurposeCode":"TAXS","proprietaryPurpose":"190401.21","referredDocument":[],"unstructured":"1Назначение платежа в неструктурированной виде длиной 140 символов 12Назначение платежа в неструктурированной виде длиной 140 символов 23Назначение платежа в неструктурированной виде длиной 140 символов 3"},"requestedExecutionDate":"2024-10-09","taxRemittance":{"additionalInformation":"a","category":"11902","creditor":{"taxIdentification":"INN101103846"},"date":"2025-08-13","debtor":{"taxIdentification":"INP511111111"},"fromDate":"2025-07-01","rate":"20.0","referenceNumber":"123","taxableBaseAmount":"1000.00","toDate":"2025-07-31","totalAmount":"200.00","type":"MM07","ultimateDebtor":{"name":"a","taxIdentification":"INP511111111"},"year":"2025-01-01"},"ultimateCreditor":{"countryOfResidence":"BY","name":"Любанский сельский исполком","organisationIdentification":[{"code":"TXID","identification":"INN400031143"}],"privateIdentification":[]}},"link":"https://dc.asb.by/paymentСonsents/domesticTax/447d4b11-6097-4f548073-1c21960c9661","status":"AwaitingAuthorisation","statusUpdateDateTime":"2025-11-05T16:56:47.263849771+03:00"},"links":{"self":"https://sc-map-testversion-vip.softclub.by:8008/oapi-channel/open-banking/v1.0/paymentConsents/domesticTax/447d4b11-6097-4f548073-1c21960c9661"},"meta":{"totalPages":1}}
    console.log(JSON.stringify(createPaymentConsentsDomesticTaxResponse))
    await paf.appendToDefinedFile("logs.txt","createPaymentConsentsDomesticTaxResponse",JSON.stringify(createPaymentConsentsDomesticTaxResponse))

    // let requestObject = await generateRequestPISPauthorisationCode(config,createPaymentConsentsDomesticTaxResponse.data.domesticTaxConsentId)
    // console.log(JSON.stringify(requestObject))
    // await paf.appendToDefinedFile("logs.txt","requestObject_response",JSON.stringify(requestObject))

    // let accListPayments = await paf.getAccListPaymentsKEYCLOAK(config,"","V087_TEST1",config.client_id_pisp,createPaymentConsentsDomesticTaxResponse.data.domesticTaxConsentId)
    // console.log(JSON.stringify(accListPayments))
    // await paf.appendToDefinedFile("logs.txt","accListPayments",JSON.stringify(accListPayments))

    // let accListcheck = await paf.getAccListPaymentsKEYCLOAKcheck(config,"","V087_TEST1",config.client_id_pisp,accListPayments)
    // console.log(JSON.stringify(accListcheck))
    // await paf.appendToDefinedFile("logs.txt","accListPaymentscheck",config.client_id_pisp,JSON.stringify(accListcheck))
    // let getPaymentConsentsDomesticTaxResponse = await getPaymentConsentsDomesticTax(config,pispAccessToken,createPaymentConsentsDomesticTaxResponse.data.domesticTaxConsentId)
    // console.log(JSON.stringify(getPaymentConsentsDomesticTaxResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentConsentsDomesticTaxResponse",JSON.stringify(getPaymentConsentsDomesticTaxResponse))

    // let deletePaymentConsentsDomesticTaxDBOResponse = await deletePaymentConsentsDomesticTaxDBO(config,dboClientAccessToken,createPaymentConsentsDomesticTaxResponse.data.domesticTaxConsentId)

    // let getPaymentConsentsDomesticTaxDBOResponse = await getPaymentConsentsDomesticTaxDBO(config,dboClientAccessToken,createPaymentConsentsDomesticTaxResponse.data.domesticTaxConsentId)
    // console.log(JSON.stringify(getPaymentConsentsDomesticTaxDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentConsentsDomesticTaxDBOResponse",JSON.stringify(getPaymentConsentsDomesticTaxDBOResponse))

    // let deletePaymentConsentsDomesticTaxResponse = await deletePaymentConsentsDomesticTax(config,pispAccessToken,createPaymentConsentsDomesticTaxResponse.data.domesticTaxConsentId)
    // let deletePaymentConsentsDomesticTaxDBOResponse = await deletePaymentConsentsDomesticTaxDBO(config,dboClientAccessToken,createPaymentConsentsDomesticTaxResponse.data.domesticTaxConsentId)


    // getPaymentConsentsDomesticTaxResponse = await getPaymentConsentsDomesticTax(config,pispAccessToken,createPaymentConsentsDomesticTaxResponse.data.domesticTaxConsentId)
    // console.log(JSON.stringify(getPaymentConsentsDomesticTaxResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentConsentsDomesticTaxResponse",JSON.stringify(getPaymentConsentsDomesticTaxResponse))

    // let accListPaymentsDomesticTax = await paf.getAccListPayments(config,dboClientAccessToken,createPaymentConsentsDomesticTaxResponse.data.domesticTaxConsentId)
    // console.log(JSON.stringify(accListPaymentsDomesticTax))
    // await paf.appendToDefinedFile("logs.txt","accListPaymentsDomesticTax",JSON.stringify(accListPaymentsDomesticTax))

    // let patchPaymentConsentDomesticTaxResponse = await patchPaymentConsentDomesticTax(config,dboClientAccessToken,accListPaymentsDomesticTax)
    // const endTime = performance.now()
    // console.log(`Call to doSomething took ${endTime - startTime} milliseconds`)
    // console.log(JSON.stringify(patchPaymentConsentDomesticTaxResponse))
    // await paf.appendToDefinedFile("logs.txt","patchPaymentConsentDomesticTaxResponse",JSON.stringify(patchPaymentConsentDomesticTaxResponse))
    // let deletePaymentConsentsDomesticTaxDBOResponse = await deletePaymentConsentsDomesticTaxDBO(config,dboClientAccessToken,createPaymentConsentsDomesticTaxResponse.data.domesticTaxConsentId)
    // await modifyPostPaymentsDomesticTaxBody(createPaymentConsentsDomesticTaxResponse.data.domesticTaxConsentId,createPaymentConsentsDomesticTaxResponse.data.initiation)
    // let postPaymentsDomesticTaxResponse = await postPaymentsDomesticTax(config,pispAccessToken)
    // console.log(JSON.stringify(postPaymentsDomesticTaxResponse))
    // await paf.appendToDefinedFile("logs.txt","createPaymentsDomesticTaxResponse",JSON.stringify(postPaymentsDomesticTaxResponse))

    // let getPaymentConsentsDomesticTaxDBOResponse = await getPaymentConsentsDomesticTaxDBO(config,dboClientAccessToken,createPaymentConsentsDomesticTaxResponse.data.domesticTaxConsentId)
    // console.log(JSON.stringify(getPaymentConsentsDomesticTaxDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentConsentsDomesticTaxDBOResponse",JSON.stringify(getPaymentConsentsDomesticTaxDBOResponse))

    // let domesticTaxId = postPaymentsDomesticTaxResponse.data.domesticTaxId
    // let domesticTaxId = "795ABSB2025110300000000002746863333"
    // let getPaymentsDomesticTaxDBOResponse = await getPaymentsDomesticTaxDBO(config,dboClientAccessToken,postPaymentsDomesticTaxResponse.data.domesticTaxId)
    // let getPaymentsDomesticTaxDBOResponse = await getPaymentsDomesticTaxDBO(config,dboClientAccessToken,domesticTaxId)
    // console.log(JSON.stringify(getPaymentsDomesticTaxDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentDomesticTaxDBOResponse",JSON.stringify(getPaymentsDomesticTaxDBOResponse))

    // let domesticTaxId = "795ABSB2025110300000000002746760000"
    // let getPaymentsDomesticTaxResponse = await getPaymentsDomesticTax(config,dboClientAccessToken,domesticTaxId)
    // console.log(JSON.stringify(getPaymentsDomesticTaxResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentDomesticTaxResponse",JSON.stringify(getPaymentsDomesticTaxResponse))

    // await deletePaymentsDomesticTaxDBO(config,dboClientAccessToken,domesticTaxId)

    // let getPaymentsDomesticTaxDBOResponse = await getPaymentsDomesticTaxDBO(config,dboClientAccessToken,createPaymentConsentsDomesticTaxResponse.data.domesticTaxConsentId)
    // console.log(JSON.stringify(getPaymentsDomesticTaxDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentDomesticTaxDBOResponse",JSON.stringify(getPaymentsDomesticTaxDBOResponse))

    // await deletePaymentsDomesticTax(config,dboClientAccessToken,domesticTaxId)
    // console.log(JSON.stringify(deletePaymentsDomesticTaxResponse))
    // await paf.appendToDefinedFile("logs.txt","deletePaymentDomesticTaxResponse",JSON.stringify(deletePaymentsDomesticTaxResponse))

    // let getPaymentsDomesticTaxDBOResponse = await getPaymentsDomesticTaxDBO(config,dboClientAccessToken,domesticTaxId)
    // console.log(JSON.stringify(getPaymentsDomesticTaxDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentDomesticTaxDBOResponse",JSON.stringify(getPaymentsDomesticTaxDBOResponse))
    // console.log(`Call to doSomething took ${endTime - startTime} milliseconds`)
}

main1()