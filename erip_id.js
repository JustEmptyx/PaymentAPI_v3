    ...
    */
    const bankParams = Object.freeze({
        TVdigitalChannels:   {
            ...
            ERIP_BEARER_DATA: "client_id=ERIP&client_secret=meGShQhM3hX28JJ7qr8sJ0aIGxVdxdGR&grant_type=client_credentials&scope=SC-APPS instant-payments openid",
            REALM_NAME: "SCRealm",
        },
        TViBankASB: {
            BASE_URL : "https://sc-map-testversion-vip.softclub.by:8008/",
...
async function getPOSTconsentInstantBodyResponse(){}
async function calcAll(config){
    const eripBearerToken = (await getBearerToken(config.BASE_URL + "auth/realms/" + config.REALM_NAME + "/protocol/openid-connect/token", config.ERIP_BEARER_DATA)).access_token
    ...
    const accountsListByLoginResponse = await getAccountsListByLogin(config.BASE_URL + 'oapi-channel/open-banking/v1.0/accountsList/login/' + config.CLIENT_NAME + '/accountConsents/' + config.consent_id,obClientCredentialsBearerToken)

    console.log(JSON.stringify(accountsListByLoginResponse))

    const postConsentInstantBody = generatePOSTconsentInstantBody(accountsListByLoginResponse)
    console.log(JSON.stringify(postConsentInstantBody))
    const jwsSignature = await getJWSsignature(postConsentInstantBody,config)
    console.log(jwsSignature)

    const postConsentInstantResponse = await postConsentInstant(postConsentInstantBody,config,eripBearerToken,jwsSignature)
    console.log(postConsentInstantResponse)

    const qpispSCCryptoHashBody = await getQPISPSCCryptoHashBody(postConsentInstantResponse.data.ASPSPsession,config)
    const qpispSignature = (await makeCryptoProxyRequestHash(qpispSCCryptoHashBody,config)).ResultB64
    console.log(qpispSCCryptoHashBody)
    console.log(qpispSignature)

    const patchConsentInstantBody = await createPatchConsentInstantBody(qpispSCCryptoHashBody.DataB64,qpispSignature)
    const patchConsentInstantResponse = await patchConsentInstant(patchConsentInstantBody,config,eripBearerToken)
    console.log(patchConsentInstantResponse)
    console.log(patchConsentInstantBody)
