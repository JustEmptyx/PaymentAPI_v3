...
async function generateSignaturePOSTpaymentConsentslistPassports(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,additionalInfo = []){
  ...
  "typ": "JOSE"}
    ...
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,undefined,"listPassports")
...
async function generateSignaturePOSTpaymentslistPassports(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,additionalInfo = []){
...
async function generateSignaturePATCHpaymentConsentslistPassports(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignatureGETpaymentConsentslistPassports(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
    ...
    await paf.appendToDefinedFile("logs.txt","header",JSON.stringify(header))
    console.log("Header \r\n" +JSON.stringify(header))
    let payload = await paf.generatePayloadNew(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsConsentId,"listPassports")
    await paf.appendToDefinedFile("logs.txt","payloadPars",JSON.stringify(payload))
    console.log("Payload \r\n" +JSON.stringify(payload))
    let headerB64 = paf.convertToBase64URL(JSON.stringify(header))
    let payloadB64 = paf.convertToBase64URL(JSON.stringify(payload))
...
async function generateSignatureGETpaymentConsentslistPassportsDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignatureGETpaymentslistPassportsDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignatureDELETEpaymentslistPassportsDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignatureGETpaymentslistPassports(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignatureDELETEpaymentslistPassports(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignatureDELETEpaymentConsentslistPassports(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignatureDELETEpaymentConsentslistPassportsDBO(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignaturePUTcreateExternalRepresentationlistPassports(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function generateSignaturePUTcreateExternalRepresentationSpecialPartlistPassports(config,method,token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsConsentId,additionalInfo = []){
    let header = await paf.generateHeader(config,additionalInfo)
    if(authType == "OBclientCredentials"){
    header["http://openbanking.asb.by/signedData"].pars = [
      "@method",
...
async function createPaymentConsentslistPassports(config,access_token){
    let requestBodyName = "POSTpaymentConsents/listPassports1"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("listPassports",requestBodyName))
    ...
    let method = "POSTpaymentConsentslistPassports"
    let signature = await generateSignaturePOSTpaymentConsentslistPassports(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName)
    ...
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/listPassports", {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "POST",
    mode: "cors",
    headers: headersList,
    body: JSON.stringify(paf.sortObjectAlphabeticallyNew(paf.getRequestBody("listPassports",requestBodyName)))
  });
  await paf.appendToDefinedFile("logs.txt","fullRequest","method: POST \r\n mode: cors \r\n headers: " + JSON.stringify(headersList) + "\r\n" + "body: " + JSON.stringify(paf.getRequestBody("listPassports",requestBodyName)))
  return response.json();
}

async function postPaymentslistPassports(config,access_token){
    let requestBodyName = "POSTpaymentslistPassports1"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("listPassports",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "POSTpaymentslistPassports"
    let signature = await generateSignaturePOSTpaymentslistPassports(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
    ...
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/listPassports", {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "POST",
    mode: "cors",
    headers: headersList,
...
async function generateSpecPartlistPassports(config,requestBodyWithExternalRepresentation){
    let imitIns = await paf.createImitationInsert(config,requestBodyWithExternalRepresentation)
    let specPart = await createSpecialPartObjectlistPassports(config,imitIns.ResultB64)
    console.log("specialPart \n" + JSON.stringify(specPart))
    setRequestBody("listPassports","PUTpaymentConsentsCreateExternalRepresentationSpecialPartlistPassports",specPart)
    let externalRepresentationSpecialPart = await createExternalRepresentationSpecialPartlistPassports(config,specPart)
...
async function createSpecialPartObjectlistPassports(config,imitIns){
...
async function createExternalRepresentationlistPassports(config, requestBody){
    await paf.appendToDefinedFile("logs.txt","EXTERNAL REPRESENTATION","")
    let token = await paf.createDboClientToken(config)
    let access_token = token.access_token

    let requestBodyName = "PUTpaymentConsentsCreateExternalRepresentationlistPassports"
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody = {
        "Auth":{
            "CryptoType":1,
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "PUTpaymentConsentsCreateExternalRepresentationlistPassports"
    let signature = await generateSignaturePUTcreateExternalRepresentationlistPassports(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,["NoIdempotencyKey"])
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
  ...
  // const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/listPassports/createExternalRepresentation", {
  //   method: "PUT",
  //   mode: "cors",
  //   headers: headersList,
...
async function createExternalRepresentationSpecialPartlistPassports(config, requestBody){
    await paf.appendToDefinedFile("logs.txt","EXTERNAL REPRESENTATION SPECIAL PART","")
    let token = await paf.createDboClientToken(config)
    let access_token = token.access_token

    let requestBodyName = "PUTpaymentConsentsCreateExternalRepresentationSpecialPartlistPassports"
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody = {
        "Auth":{
            "CryptoType":1,
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "PUTpaymentConsentsExternalRepresentationSpecialPartlistPassports"
    let signature = await generateSignaturePUTcreateExternalRepresentationSpecialPartlistPassports(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,["NoIdempotencyKey"])
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
...
async function generatePaymentConsentlistPassportsAuthoriseBody(config,accList,createPaymentConsentslistPassportsResponse){
    ...
    setRequestBody("listPassports","PUTpaymentConsentsCreateExternalRepresentationlistPassports",requestBody)
    let latestAuthType = authType;
    authType = "OBclientCredentials"
    let externalRepresentation = await createExternalRepresentationlistPassports(config, requestBody)
    ...
    authType = "OBclientCredentials"
    let specPart = await generateSpecPartlistPassports(config, requestBody)
    authType = latestAuthType
    requestBody.specialPart = specPart.specialPart
    requestBody = paf.renameKeyInObject(requestBody,"paymentConsentId","listPassportsConsentId")
    requestBody = paf.sortObjectAlphabeticallyNew(requestBody)

    return requestBody
}

async function patchPaymentConsentlistPassports(config,access_token,accList,createPaymentConsentslistPassportsResponse){
    let requestBodyName = "PATCHpaymentConsents/listPassports1"
    let requestBody = await generatePaymentConsentlistPassportsAuthoriseBody(config,accList,createPaymentConsentslistPassportsResponse)
    setRequestBody("listPassports",requestBodyName,requestBody)
    let savedBody = paf.getRequestBody("listPassports",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
        cryptoHashBody = {
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "PATCHpaymentConsentslistPassports"
    let signature = await generateSignaturePATCHpaymentConsentslistPassports(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
...
async function getPaymentConsentslistPassports(config,access_token,listPassportsConsentId){
    let requestBodyName = "GETpaymentConsents/listPassports"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("listPassports",requestBodyName))
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentConsentslistPassports"
    let signature = await generateSignatureGETpaymentConsentslistPassports(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
    ...
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/listPassports/" + listPassportsConsentId, {
    // const response = await fetch("http://192.168.166.214:9100/openbanking/paymentConsents/domestic",{
    method: "GET",
    mode: "cors",
    headers: headersList,
...
async function getPaymentConsentslistPassportsDBO(config,access_token,listPassportsConsentId){
    let latestAuthType = authType
    let requestBodyName = "GETpaymentConsents/listPassportsDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("listPassports",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    authType = "OBclientCredentials"
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentConsentslistPassportsDBO"
    let signature = await generateSignatureGETpaymentConsentslistPassportsDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
    ...
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/paymentConsents/listPassports/" + listPassportsConsentId +"/PSUorPAU/V087_TEST1", {
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
...
async function getPaymentslistPassportsDBO(config,access_token,listPassportsId){
    let latestAuthType = authType
    let requestBodyName = "GETpayments/listPassportsDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("listPassports",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    authType = "OBclientCredentials"
    paf.setAuthType("OBclientCredentials")
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentslistPassportsDBO"
    let signature = await generateSignatureGETpaymentslistPassportsDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
    ...
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/listPassports/" + listPassportsId +"/PSUorPAU/V087_TEST1", {
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
...
async function deletePaymentslistPassportsDBO(config,access_token,listPassportsId){
    let latestAuthType = authType
    let requestBodyName = "DELETEpayments/listPassportsDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("listPassports",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    authType = "OBclientCredentials"
    paf.setAuthType("OBclientCredentials")
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentslistPassportsDBO"
    let signature = await generateSignatureDELETEpaymentslistPassportsDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList ={
      "Content-Type": "application/json;charset=utf-8",
...
async function getPaymentslistPassports(config,access_token,listPassportsId){
    let requestBodyName = "GETpayments/listPassports"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("listPassports",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "GETpaymentslistPassports"
    let signature = await generateSignatureGETpaymentslistPassports(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
    ...
    await paf.appendToDefinedFile("logs.txt","headersList",JSON.stringify(headersList))
    console.log(JSON.stringify(headersList))
    const response = await fetch(config.url_swagger + "oapi-channel/open-banking/v1.0/payments/listPassports/" + listPassportsId, {
    method: "GET",
    mode: "cors",
    headers: headersList,
  });
...
async function deletePaymentslistPassports(config,access_token,listPassportsId){
    let requestBodyName = "DELETEpayments/listPassports"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("listPassports",requestBodyName))
    // let requestBody = getRequestBody("pispAuth",requestBodyName)
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentslistPassports"
    let signature = await generateSignatureDELETEpaymentslistPassports(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
    let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
...
async function deletePaymentConsentslistPassports(config,access_token,listPassportsConsentId){
    let requestBodyName = "DELETEpaymentConsentslistPassports"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("listPassports",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentConsentslistPassports"
    let signature = await generateSignatureDELETEpaymentConsentslistPassports(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
      let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
...
async function deletePaymentConsentslistPassportsDBO(config,access_token,listPassportsConsentId){
    let prevAuthType = authType
    authType = "OBclientCredentials"
    setAuthType("OBclientCredentials")
    let requestBodyName = "DELETEpaymentConsentslistPassportsDBO"
    let requestBody = paf.sortObjectAlphabeticallyNew(paf.getRequestBody("listPassports",requestBodyName))
    await paf.appendToDefinedFile("logs.txt","requestBody",JSON.stringify(requestBody))
    let cryptoHashBody
    if(requestBody!= ""){
    ...
    let fapiInteractionId = uuid.v4()
    let idempotencyKey = uuid.v4()
    let method = "DELETEpaymentConsentslistPassportsDBO"
    let signature = await generateSignatureDELETEpaymentConsentslistPassportsDBO(config,method,access_token,fapiInteractionId,idempotencyKey,requestBodyName,listPassportsConsentId)
    await paf.appendToDefinedFile("logs.txt","signature",JSON.stringify(signature))
    console.log(JSON.stringify(signature))
      let headersList = authType == 'OBclientCredentials' ? {
      "Content-Type": "application/json;charset=utf-8",
...
async function modifyPATCHpaymentConsentlistPassportsBody(uri_paymentLink,instantConsentId,invoiceInstantId,instructionIdentification,endToEndIdentification){
...
async function modifyPostPaymentslistPassportsBody(listPassportsConsentId,initiation){
    let body = await paf.getRequestBody("listPassports","POSTpaymentslistPassports1")
    body.data.listPassportsConsentId = listPassportsConsentId
    body.data.initiation = initiation
    await paf.appendToDefinedFile("logs.txt","POSTpaymentslistPassports1_modifiedBody",JSON.stringify(body))
    await setRequestBody("listPassports","POSTpaymentslistPassports1",body)
}
...
async function main1() {
    ...
    let createPaymentConsentslistPassportsResponse = await createPaymentConsentslistPassports(config,pispAccessToken)
    // let createPaymentConsentslistPassportsResponse = {"data":{"charge":[],"creationDateTime":"2025-10-20T19:19:23.514437627+03:00","initiation":{"amount":"201.00","creditor":{"countryOfResidence":"BY","name":"ST-”KRYSHKOVSKAYA short“ короткое наименование","organisationIdentification":[{"code":"TXID","identification":"INN101103846"}],"privateIdentification":[]},"creditorAccount":{"identification":"BY48AKBB30121272200458000933","schemeName":"BY.NBRB.IBAN"},"creditorAgent":{"identification":"AKBBBY2X","name":"ОАО 'АСБ Беларусбанк'"},"currency":"BYN","endToEndIdentification":"01.20250412.8bd17819bcf8d8da","instructionIdentification":"795SDBO20250412E5AE1D70F25F431F84DD","listPassports":[{"IBAN":"BY21AKBB30140012426250070000","amount":"100.00","name":"Белуга Митрофан Доромидонтович"},{"IBAN":"BY06AKBB30140012426260070000","amount":"101.00","name":"Килька Авдотья Никитишна"}],"localInstrument":"BY.NBRB.BISS.NORMAL","requestedExecutionDate":"2025-12-12"},"link":"https://dc.asb.by/paymentСonsents/listPassports/15582e04-8249-4957a675-0ba7c95535fb","listPassportsConsentId":"15582e04-8249-4957a675-0ba7c95535fb","status":"AwaitingAuthorisation","statusUpdateDateTime":"2025-10-20T19:19:23.514437627+03:00"},"links":{"self":"https://sc-map-testversion-vip.softclub.by:8008/oapi-channel/open-banking/v1.0/paymentConsents/listPassports/15582e04-8249-4957a675-0ba7c95535fb"},"meta":{"totalPages":1}}
    console.log(JSON.stringify(createPaymentConsentslistPassportsResponse))
    await paf.appendToDefinedFile("logs.txt","createPaymentConsentslistPassportsResponse",JSON.stringify(createPaymentConsentslistPassportsResponse))

    // let getPaymentConsentslistPassportsResponse = await getPaymentConsentslistPassports(config,pispAccessToken,createPaymentConsentslistPassportsResponse.data.listPassportsConsentId)
    // console.log(JSON.stringify(getPaymentConsentslistPassportsResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentConsentslistPassportsResponse",JSON.stringify(getPaymentConsentslistPassportsResponse))

    // let getPaymentConsentslistPassportsDBOResponse = await getPaymentConsentslistPassportsDBO(config,dboClientAccessToken,createPaymentConsentslistPassportsResponse.data.listPassportsConsentId)
    // let getPaymentConsentslistPassportsDBOResponse = await getPaymentConsentslistPassportsDBO(config,dboClientAccessToken,"17f75afe-672a-4087979d-e8c65da1e6bb")
    // console.log(JSON.stringify(getPaymentConsentslistPassportsDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentConsentslistPassportsDBOResponse",JSON.stringify(getPaymentConsentslistPassportsDBOResponse))

    // let deletePaymentConsentslistPassportsResponse = await deletePaymentConsentslistPassports(config,pispAccessToken,createPaymentConsentslistPassportsResponse.data.listPassportsConsentId)
    // let deletePaymentConsentslistPassportsDBOResponse = await deletePaymentConsentslistPassportsDBO(config,dboClientAccessToken,createPaymentConsentslistPassportsResponse.data.listPassportsConsentId)

    // getPaymentConsentslistPassportsResponse = await getPaymentConsentslistPassports(config,pispAccessToken,createPaymentConsentslistPassportsResponse.data.listPassportsConsentId)
    // console.log(JSON.stringify(getPaymentConsentslistPassportsResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentConsentslistPassportsResponse",JSON.stringify(getPaymentConsentslistPassportsResponse))
    //
    // let accListPaymentslistPassports = await paf.getAccListPayments(config,dboClientAccessToken,createPaymentConsentslistPassportsResponse.data.listPassportsConsentId)
    // console.log(JSON.stringify(accListPaymentslistPassports))
    // await paf.appendToDefinedFile("logs.txt","accListPaymentslistPassports",JSON.stringify(accListPaymentslistPassports))


    // let patchPaymentConsentlistPassportsResponse = await patchPaymentConsentlistPassports(config,dboClientAccessToken,accListPaymentslistPassports,createPaymentConsentslistPassportsResponse)
    // console.log(JSON.stringify(patchPaymentConsentlistPassportsResponse))
    // await paf.appendToDefinedFile("logs.txt","patchPaymentConsentlistPassportsResponse",JSON.stringify(patchPaymentConsentlistPassportsResponse))

    // await modifyPostPaymentslistPassportsBody(createPaymentConsentslistPassportsResponse.data.listPassportsConsentId,createPaymentConsentslistPassportsResponse.data.initiation)
    // let postPaymentslistPassportsResponse = await postPaymentslistPassports(config,pispAccessToken)
    // console.log(JSON.stringify(postPaymentslistPassportsResponse))
    // await paf.appendToDefinedFile("logs.txt","createPaymentslistPassportsResponse",JSON.stringify(postPaymentslistPassportsResponse))

    // let listPassportsId = postPaymentslistPassportsResponse.data.listPassportsId
    // let getPaymentslistPassportsDBOResponse = await getPaymentslistPassportsDBO(config,dboClientAccessToken,listPassportsId)
    // console.log(JSON.stringify(getPaymentslistPassportsDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentlistPassportsDBOResponse",JSON.stringify(getPaymentslistPassportsDBOResponse))

    // let getPaymentslistPassportsResponse = await getPaymentslistPassports(config,dboClientAccessToken,listPassportsId)
    // console.log(JSON.stringify(getPaymentslistPassportsResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentlistPassportsResponse",JSON.stringify(getPaymentslistPassportsResponse))

    // await deletePaymentslistPassportsDBO(config,dboClientAccessToken,listPassportsId)
    // console.log(JSON.stringify(deletePaymentslistPassportsDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","deletePaymentlistPassportsDBOResponse",JSON.stringify(deletePaymentslistPassportsDBOResponse))

    // getPaymentslistPassportsDBOResponse = await getPaymentslistPassportsDBO(config,dboClientAccessToken,listPassportsId)
    // console.log(JSON.stringify(getPaymentslistPassportsDBOResponse))
    // await paf.appendToDefinedFile("logs.txt","getPaymentlistPassportsDBOResponse",JSON.stringify(getPaymentslistPassportsDBOResponse))

    // await deletePaymentslistPassports(config,dboClientAccessToken,listPassportsId)
    // console.log(JSON.stringify(deletePaymentslistPassportsResponse))
    // await paf.appendToDefinedFile("logs.txt","deletePaymentlistPassportsResponse",JSON.stringify(deletePaymentslistPassportsResponse))
